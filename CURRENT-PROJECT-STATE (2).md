...
Dari total **25 ADR** di `architecture-decision-records.md`, **22 ADR berstatus Approved** (termasuk ADR-001 dan ADR-005 yang baru diselesaikan):
...
| ADR-005 | Search Strategy | **Approved (baru)** | PostgreSQL FTS + `pg_trgm` Fase 1, migrasi terjadwal ke Typesense Fase 2, 28 Juli 2026 |
...
**3 ADR masih berstatus OPEN** — lihat rincian di bagian *Open Decision (ADR) yang Tersisa* di bawah: **ADR-006 (Job Queue Strategy)**, **ADR-008 (Maps Provider)**, **ADR-018 (Caching Strategy)**. Tidak satu pun dari ketiganya memblokir Sprint S0.
...
**Ringkasan:** 1 dari 14 baris selesai (dokumentasi/governance). Implementasi kode: **0% secara keseluruhan.** Tidak ada perubahan fase implementasi pada sesi ini — pembaruan sesi ini murni governance (resolusi ADR-005, menyusul ADR-001 pada sesi sebelumnya).
...
| 6 | Job Queue (BullMQ vs Supabase Edge Functions) belum masuk *Official Technology Stack*. | `technology-decisions.md` Bagian 9 poin 2 | Open — terkait **ADR-006** (lihat *Open Decision* di bawah) |
...
> Sumber: `architecture-decision-records.md` Bagian 4, 7 (Impact Analysis), dan 8 (Implementation Order). Ketiga ADR berikut masih **OPEN** — AI Coding Assistant **tidak boleh berasumsi** salah satu opsi sudah dipilih untuk area ini.
...
| 2 (independen, kapan pun) | **ADR-008** | Maps Provider | S4, S9 | Menunggu konfirmasi biaya dari tim bisnis — sepenuhnya independen dari ADR lain |
| 3 (terendah) | **ADR-018** | Caching Strategy (Redis) | Tidak ada sprint spesifik | Direkomendasikan diputuskan **setelah** ADR-006, agar keputusan Redis (jika ada) diambil satu kali untuk kedua kebutuhan |

**Tidak ada satu pun dari ketiga ADR ini yang memblokir Sprint S0.** ADR-006 wajib diselesaikan sebelum Sprint S6/S13; ADR-008 sebelum Sprint S4/S9 (khususnya form lokasi & Developer Directory). **ADR-005 (Search Strategy) telah diselesaikan** (Approved, 28 Juli 2026) dan tidak lagi termasuk dalam tabel ini — lihat *ADR & Governance Snapshot* di atas. Sesuai `architecture-decision-records.md` Bagian 10 (AI Usage Rules), AI **wajib berhenti dan meminta keputusan eksplisit** dari Technical Lead/manusia berwenang jika sebuah task menyentuh area yang dinaungi ADR berstatus OPEN — bukan memilih sendiri salah satu opsi.
...
| Technology Decisions | **Not Ready** — status dokumen masih "Draft"; 2 sub-keputusan (Maps/ADR-008, Job Queue/ADR-006) belum tuntas; Search Engine/ADR-005 kini **tuntas** |
...
**Kesimpulan readiness terbaru:** 2 dari 6 kondisi GO WITH CONDITIONS kini terpenuhi (kondisi #1 dan #3 — keduanya sebelumnya dinilai sebagai blocker arsitektur berkategori Critical/High). Status proyek secara keseluruhan **tetap GO WITH CONDITIONS** — Sprint S0 (scaffolding murni) tidak pernah terblokir oleh kondisi manapun dan tetap dapat dimulai kapan saja; Sprint S1 ke atas (menyentuh backend/API/database) tetap menunggu penyelesaian kondisi #2, #4, #5, dan #6, khususnya #2 dan #5 yang berdampak langsung ke migration Sprint S0.
...
4. **Kini tidak lagi ada risiko drift arsitektur backend maupun search.** Sebelumnya, memulai Sprint S0 dengan ADR-001 dan ADR-005 masih OPEN membawa risiko sesi AI berbeda mengasumsikan pola backend/search yang berbeda-beda; risiko ini sudah hilang menyusul ADR-001 Approved (27 Juli 2026) dan ADR-005 Approved (28 Juli 2026) — Sprint S0 dapat dieksekusi dengan konteks backend & search yang sudah pasti (Route Handlers + Supabase, tanpa `/apps/api` terpisah; PostgreSQL FTS + `pg_trgm` untuk search Fase 1).
...
13. **Jika sebuah task menyentuh area yang dinaungi ADR berstatus OPEN** (ADR-006 Job Queue, ADR-008 Maps, ADR-018 Caching) — **berhenti dan minta keputusan eksplisit**, jangan memilih sendiri salah satu opsi (lihat `architecture-decision-records.md` Bagian 10, AI Usage Rules poin 4). **ADR-005 (Search Strategy) sudah Approved** — task pencarian listing Fase 1 (PostgreSQL FTS + `pg_trgm`) tidak lagi termasuk area ini.
