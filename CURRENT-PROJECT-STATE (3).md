...
| **Versi** | 0.1 (Pra-Development — belum ada rilis kode; tidak berubah dari snapshot sebelumnya karena tidak ada perubahan kode nyata pada sesi ini, hanya keputusan governance) |
| **Tanggal Update** | 29 Juli 2026 |
| **Status** | 🟡 **Perencanaan & Dokumentasi Selesai — Implementasi Teknis Belum Dimulai.** Tiga blocker arsitektur tertinggi (ADR-001 — Backend Architecture, ADR-005 — Search Strategy, ADR-006 — Job Queue Strategy) telah **diselesaikan (Approved, 27, 28 & 29 Juli 2026)**; **tidak ada lagi Open Decision yang memblokir dimulainya Sprint S0.** |
| **Development Phase** | **Pre-Phase 0** — seluruh dokumen governance (Constitution, PRD, ERD, API Spec, User Flow, SEO Spec, System Architecture, Technology Decisions, Dependency Manifest, AI Development Blueprint, AI Context Pack, Development Roadmap, Task Template, **Architecture Decision Records**) sudah final/tersedia. **Sprint S0 (Foundation Infrastructure) belum dieksekusi** — belum ada monorepo, belum ada kode, belum ada database fisik. **Tidak ada perubahan fase pada sesi ini** (implementasi kode tetap 0%) — pembaruan bersifat governance/keputusan arsitektur (resolusi ADR-006), bukan transisi fase. |
| **Milestone Berikutnya** | **Sprint S0 — Foundation Infrastructure.** Tidak berubah dari sebelumnya sebagai modul berikutnya yang direkomendasikan; kini secara resmi **tidak lagi memiliki dependency terhadap Open Decision arsitektur backend, search, maupun job queue** (ADR-001, ADR-005, dan ADR-006 sudah Approved) — lihat bagian *ADR & Governance Snapshot* dan *Next Recommended Module*. |

---

# ADR & Governance Snapshot

> Bagian baru — merangkum status **Architecture Decision Records** (`architecture-decision-records.md`, v1.0) per 27–29 Juli 2026. Rincian penuh setiap ADR (Context/Decision/Rationale/Consequences) tetap berada di dokumen sumber; bagian ini hanya mencatat **status**, bukan menggandakan isinya.
...
**ADR-005 — Search Strategy** — **Status naik dari OPEN menjadi APPROVED (dengan catatan)** pada **28 Juli 2026**, melalui sesi Architecture Review Board.

- **Keputusan:** Strategi bertahap — PostgreSQL Full-Text Search + ekstensi `pg_trgm` sebagai mesin pencari Fase 1 (MVP), tanpa komponen infrastruktur tambahan di luar Supabase; migrasi terjadwal ke **Typesense** di Fase 2 begitu salah satu dari tiga kriteria ambang tercapai (volume listing aktif >±50.000, latensi p95 `/properties/search` >500ms, atau keluhan relevansi berulang ≥3 laporan independen/sprint).
- **Cross-reference:** Dicatat ulang sebagai **ADR-039** di `decision-log.md` (dua rangkaian penomoran ADR yang berbeda, mengacu topik yang sama — bukan hubungan Supersedes/Superseded By).
- **Catatan kondisional dari Board (belum ditutup):**
  1. Proyeksi volume listing realistis 6–12 bulan pertama perlu dikonfirmasi tim bisnis untuk memvalidasi/menyesuaikan angka ambang 50.000 baris yang dipakai sebagai baseline awal.
  2. Kapasitas DevOps/anggaran untuk Typesense di masa depan (self-hosted vs Typesense Cloud) perlu dikonfirmasi sebelum kriteria ambang migrasi tercapai.
- **Dampak langsung ke dokumen ini:** Menghapus baris Search Engine dari *Known Technical Debt* dan dari *Open Decision (ADR) yang Tersisa* (lihat kedua bagian tsb di bawah, sudah diperbarui); memperbarui kondisi #3 di *Readiness Snapshot*.

**ADR-006 — Job Queue Strategy** — **Status naik dari OPEN menjadi APPROVED (dengan catatan)** pada **29 Juli 2026**, melalui sesi Architecture Review Board.

- **Keputusan:** Strategi hybrid native — tugas terjadwal periodik (reminder H-1, scan listing stale >90 hari, reminder customer/jadwal temu, fallback sitemap regeneration) dijalankan via **Vercel Cron Jobs**; tugas event-driven instan (counter sync, sitemap regeneration saat publish) dijalankan via **Postgres Trigger/Database Webhook** — seluruhnya dalam satu `apps/web`, tanpa runtime/service tambahan. Migrasi terjadwal ke **QStash (Upstash)** di Fase 2 begitu salah satu dari tiga kriteria ambang tercapai (volume job harian melampaui kapasitas batching per invocation, kebutuhan retry/backoff/dead-letter kompleks, atau frekuensi melampaui batas cron interval tier Vercel). **BullMQ+Redis ditolak** — worker long-running-nya tidak kompatibel dengan model serverless Vercel yang dikunci ADR-001.
- **Cross-reference:** Dicatat ulang sebagai **ADR-040** di `decision-log.md` (dua rangkaian penomoran ADR yang berbeda, mengacu topik yang sama — bukan hubungan Supersedes/Superseded By).
- **Catatan kondisional dari Board (belum ditutup):**
  1. Tier Vercel yang akan dipakai di produksi (Hobby/Pro/Enterprise) perlu dikonfirmasi — menentukan batas jumlah dan frekuensi minimum Cron Jobs yang tersedia.
  2. Status resmi fitur Agent Workspace di roadmap (reminder listing >90 hari, jadwal temu, reminder customer) perlu dikonfirmasi tim produk — jika masuk roadmap, memerlukan ADR/PRD update terpisah.
- **Dampak langsung ke dokumen ini:** Menghapus baris Job Queue dari *Known Technical Debt* dan dari *Open Decision (ADR) yang Tersisa* (lihat kedua bagian tsb di bawah, sudah diperbarui); memperbarui kondisi #4 di *Readiness Snapshot*.

## Keputusan yang Telah Selesai (Approved)

Dari total **25 ADR** di `architecture-decision-records.md`, **23 ADR berstatus Approved** (termasuk ADR-001, ADR-005, dan ADR-006 yang baru diselesaikan):

| ADR | Topik | Status | Catatan |
|---|---|---|---|
| ADR-001 | Backend Architecture | Approved | Next.js Route Handlers + Supabase, 27 Juli 2026 |
...
| ADR-005 | Search Strategy | Approved | PostgreSQL FTS + `pg_trgm` Fase 1, migrasi terjadwal ke Typesense Fase 2, 28 Juli 2026 |
| ADR-006 | Job Queue Strategy | **Approved (baru)** | Vercel Cron Jobs + Postgres Trigger/Webhook Fase 1, migrasi terjadwal ke QStash Fase 2, 29 Juli 2026 |
...
**2 ADR masih berstatus OPEN** — lihat rincian di bagian *Open Decision (ADR) yang Tersisa* di bawah: **ADR-008 (Maps Provider)**, **ADR-018 (Caching Strategy)**. Tidak satu pun dari keduanya memblokir Sprint S0.
...
**Ringkasan:** 1 dari 14 baris selesai (dokumentasi/governance). Implementasi kode: **0% secara keseluruhan.** Tidak ada perubahan fase implementasi pada sesi ini — pembaruan sesi ini murni governance (resolusi ADR-006, menyusul ADR-001 dan ADR-005 pada sesi-sesi sebelumnya).
...
> Kontrak **target** endpoint sudah didefinisikan lengkap di `API-Specification-Real-Estate-Agency-Platform-v1.1.md` (13 bagian: Auth, Property/Listing, Search, Lead Generation, Communication, Financial/DBR Calculator, Notification, Wilayah, Integrasi Pihak Ketiga, SEO/Analytics, Modul Pendukung). Ini adalah **spesifikasi**, bukan endpoint yang sudah aktif. Pola implementasinya kini **terkunci** sebagai Route Handlers (`app/api/v1/**/route.ts`) menyusul ADR-001 (Approved, 27 Juli 2026) — tidak ada lagi opsi bercabang "Route Handlers vs service terpisah". Mesin di balik `GET /properties/search` & `GET /properties/autocomplete` juga sudah **terkunci** menyusul ADR-005 (Approved, 28 Juli 2026) — PostgreSQL Full-Text Search + `pg_trgm`, tanpa opsi bercabang Typesense/Elasticsearch di Fase 1. Mekanisme di balik proses asinkron/terjadwal (regenerasi sitemap, reminder H-1, sinkronisasi counter) juga sudah **terkunci** menyusul ADR-006 (Approved, 29 Juli 2026) — Vercel Cron Jobs (`app/api/cron/**`) + Postgres Trigger/Database Webhook, tanpa opsi bercabang BullMQ/Redis/Edge Functions.
...
Namun tercatat beberapa **"debt keputusan governance"** (ketidaksinkronan antar dokumen desain). Status per 29 Juli 2026:
...
| 5 | ~~Search Engine (Typesense/Elasticsearch/Postgres FTS) belum masuk *Official Technology Stack*.~~ | `technology-decisions.md` Bagian 9 poin 1 (sebelumnya) | ✅ **RESOLVED** — diputuskan **Approved** via **ADR-005** / `decision-log.md` **ADR-039** (Architecture Review Board, 28 Juli 2026): PostgreSQL FTS + `pg_trgm` Fase 1, migrasi terjadwal ke Typesense Fase 2; disinkronkan ke `technology-decisions.md`, `SYSTEM-ARCHITECTURE.md`, `dependency-manifest.md`. `PROJECT-CONSTITUTION.md` sudah disinkronkan (revisi v1.3). |
| 6 | ~~Job Queue (BullMQ vs Supabase Edge Functions) belum masuk *Official Technology Stack*.~~ | `technology-decisions.md` Bagian 9 poin 2 (sebelumnya) | ✅ **RESOLVED** — diputuskan **Approved** via **ADR-006** / `decision-log.md` **ADR-040** (Architecture Review Board, 29 Juli 2026): Vercel Cron Jobs + Postgres Trigger/Database Webhook Fase 1, migrasi terjadwal ke QStash Fase 2; BullMQ+Redis ditolak karena tidak kompatibel dengan model serverless ADR-001. Disinkronkan ke `technology-decisions.md`, `SYSTEM-ARCHITECTURE.md`, `dependency-manifest.md`, `PROJECT-CONSTITUTION.md` (revisi v1.4). |
| 7 | Provider Email (Resend) & Monitoring (Sentry) sudah diputuskan di `technology-decisions.md`, belum disinkronkan ke `SYSTEM-ARCHITECTURE.md` yang masih mencatatnya kosong. | `SYSTEM-ARCHITECTURE.md` Bagian 23 poin 10–11 | Open — non-blocking |

---

# Open Decision (ADR) yang Tersisa

> Sumber: `architecture-decision-records.md` Bagian 4, 7 (Impact Analysis), dan 8 (Implementation Order). Kedua ADR berikut masih **OPEN** — AI Coding Assistant **tidak boleh berasumsi** salah satu opsi sudah dipilih untuk area ini.

| Prioritas Penyelesaian | ADR | Topik | Blocking Sprint | Rekomendasi (belum final) |
|---|---|---|---|---|
| 1 (independen, kapan pun) | **ADR-008** | Maps Provider | S4, S9 | Menunggu konfirmasi biaya dari tim bisnis — sepenuhnya independen dari ADR lain |
| 2 (terendah) | **ADR-018** | Caching Strategy (Redis) | Tidak ada sprint spesifik | Sepenuhnya independen — ADR-006 sudah resolved tanpa Redis (Vercel Cron + Postgres Trigger, bukan BullMQ), dievaluasi tersendiri untuk kebutuhan rate limiting |

**Tidak ada satu pun dari kedua ADR ini yang memblokir Sprint S0.** ADR-008 wajib diselesaikan sebelum Sprint S4/S9 (khususnya form lokasi & Developer Directory); ADR-018 tidak memblokir sprint manapun di roadmap saat ini. **ADR-005 (Search Strategy) telah diselesaikan** (Approved, 28 Juli 2026) dan **ADR-006 (Job Queue Strategy) telah diselesaikan** (Approved, 29 Juli 2026) — keduanya tidak lagi termasuk dalam tabel ini — lihat *ADR & Governance Snapshot* di atas. Sesuai `architecture-decision-records.md` Bagian 10 (AI Usage Rules), AI **wajib berhenti dan meminta keputusan eksplisit** dari Technical Lead/manusia berwenang jika sebuah task menyentuh area yang dinaungi ADR berstatus OPEN — bukan memilih sendiri salah satu opsi.

---

# Readiness Snapshot (Governance)

> Ringkasan status kesiapan per `executive-architecture-review.md` (Keputusan CTO, 27 Juli 2026) — **Final Verdict: GO WITH CONDITIONS**. Snapshot ini bersifat point-in-time dan tidak diedit ulang di dokumen sumbernya; bagian ini mencatat kondisi mana yang **sudah** dan **belum** terpenuhi per 29 Juli 2026.
...
| Technology Decisions | **Not Ready** — status dokumen masih "Draft"; 1 sub-keputusan (Maps/ADR-008) belum tuntas; Search Engine/ADR-005 dan Job Queue/ADR-006 kini **tuntas** |
...
**Kondisi GO WITH CONDITIONS (Bagian 14 `executive-architecture-review.md`) — status per 29 Juli 2026:**
...
3. ✅ Strategi Search Engine Fase 1 diputuskan (ADR-005) — **TERPENUHI** (ADR-005/ADR-039, Approved 28 Juli 2026).
4. ✅ Mekanisme Job Queue diputuskan (ADR-006) — **TERPENUHI** (ADR-006/ADR-040, Approved 29 Juli 2026).
...
**Kesimpulan readiness terbaru:** 3 dari 6 kondisi GO WITH CONDITIONS kini terpenuhi (kondisi #1, #3, dan #4 — ketiganya sebelumnya dinilai sebagai blocker arsitektur berkategori Critical/High). Status proyek secara keseluruhan **tetap GO WITH CONDITIONS** — Sprint S0 (scaffolding murni) tidak pernah terblokir oleh kondisi manapun dan tetap dapat dimulai kapan saja; Sprint S1 ke atas (menyentuh backend/API/database) tetap menunggu penyelesaian kondisi #2, #5, dan #6, khususnya #2 dan #5 yang berdampak langsung ke migration Sprint S0.
...
4. **Kini tidak lagi ada risiko drift arsitektur backend, search, maupun job queue.** Sebelumnya, memulai Sprint S0 dengan ADR-001, ADR-005, dan ADR-006 masih OPEN membawa risiko sesi AI berbeda mengasumsikan pola backend/search/job queue yang berbeda-beda; risiko ini sudah hilang menyusul ADR-001 Approved (27 Juli 2026), ADR-005 Approved (28 Juli 2026), dan ADR-006 Approved (29 Juli 2026) — Sprint S0 dapat dieksekusi dengan konteks backend, search, dan job queue yang sudah pasti (Route Handlers + Supabase, tanpa `/apps/api` terpisah; PostgreSQL FTS + `pg_trgm` untuk search Fase 1; Vercel Cron Jobs + Postgres Trigger/Database Webhook untuk job asinkron/terjadwal Fase 1).
...
13. **Jika sebuah task menyentuh area yang dinaungi ADR berstatus OPEN** (ADR-008 Maps, ADR-018 Caching) — **berhenti dan minta keputusan eksplisit**, jangan memilih sendiri salah satu opsi (lihat `architecture-decision-records.md` Bagian 10, AI Usage Rules poin 4). **ADR-005 (Search Strategy) dan ADR-006 (Job Queue Strategy) sudah Approved** — task pencarian listing Fase 1 (PostgreSQL FTS + `pg_trgm`) dan task job asinkron/terjadwal Fase 1 (Vercel Cron Jobs + Postgres Trigger/Database Webhook) tidak lagi termasuk area ini.
