...
Platform menerapkan skema **multirole (Role-Based Access Control/RBAC)** yang dapat dikonfigurasi. Hierarki akses dari tertinggi ke terendah: **Superadmin → Manager → Admin → Agen**, ditambah dua role eksternal:

| Role | Level | Deskripsi |
|---|---|---|
| **Superadmin** | Internal — tertinggi | Satu-satunya role dengan akses penuh ke **seluruh fitur web tanpa batasan**, termasuk konfigurasi sistem inti/keamanan web dan hak untuk mengatur permission seluruh role lain (Manager, Admin, Agen). Role ini tidak dapat dibatasi/dinonaktifkan. |
| **Manager** | Internal — di atas Admin | Memiliki **seluruh fungsi/akses role Admin** ditambah **akses global** untuk melihat listing dan data seluruh agen (tidak dibatasi tim/wilayah tertentu). Manager juga dapat **mengubah permission role Agen** (mis. menyesuaikan batasan fitur untuk Agen). Manager **tidak dapat** mengubah pengaturan keamanan web atau konfigurasi/fitur inti sistem, dan **tidak dapat** mengubah permission role Admin, Manager, atau Superadmin — kewenangan tersebut khusus milik Superadmin. |
| **Admin** | Internal — standar | Menjalankan operasional harian: approval agen, moderasi listing, kelola konten Learning Center/Event/Developer. Cakupan akses berada di bawah Manager, dan **tidak** dapat mengubah konfigurasi sistem inti atau hak akses role lain. |
...
| **Developer Partner** | Eksternal — terbatas | Pihak developer yang proyeknya ditampilkan untuk kerja sama agen; akses terbatas ke portal pengajuan proyek/event miliknya sendiri. |
| **Calon Pembeli (Guest/Lead)** | Publik — tanpa login | Melihat listing publik, berinteraksi dengan agen (CTA WhatsApp) tanpa perlu akun. |
...
- Login dengan email/password, opsional Google/Apple SSO
- Upload dokumen legalitas agen (KTP, NPWP, sertifikat REI/AREBI jika ada)
- Status akun: `Pending Review` → `Verified` → `Active` / `Suspended`
...
### Business Rules
- Agen tidak bisa posting listing sebelum status `Verified`.
...
- Statistik performa: jumlah listing aktif, listing terjual/tersewa, rating/review dari klien (opsional)
...
- Pembuatan/pengelolaan konten kursus (buat, edit, nonaktifkan, kelola bank soal) hanya dapat dilakukan oleh **Superadmin** dan **Admin/Instruktur**; role **Manager** secara default hanya dapat memantau progress kursus timnya (view only, tanpa hak edit konten); role **Agen** hanya dapat mendaftar (self-enroll) dan mengerjakan kursus, tidak memiliki akses kelola konten sama sekali.
...
- Superadmin/Admin/Instruktur dapat membuat, mengedit, dan menonaktifkan kursus; Manager dan Agen tidak dapat mengakses menu kelola konten ini.
...
- Superadmin/Admin dapat membuat event baru langsung tayang; Manager dan Developer Partner hanya dapat mengajukan event yang memerlukan approval.
...
- Satu proyek dapat diklaim oleh banyak agen sekaligus (non-eksklusif) kecuali dikonfigurasi eksklusif per wilayah.
...
|---|---|---|---|
| M1 Registrasi | M2 Profil Agen | Data identitas dasar agen (nama, kontak, dokumen) | Setelah status `Verified` |
...
Admin REJECT                  Admin APPROVE
   │                               │
   ▼                               ▼
Notifikasi ke agen:            Status akun → "Verified/Active"
...
--- Flow tambahan: LOGIN ---
Buka halaman Login → Input email/password (atau SSO Google/Apple)
  │
  ...
  └─ Kredensial benar ──► Cek status akun
                    ...
                    ┌───────────┼──────────────┐
                    ▼           ▼              ▼
              "Pending"    "Suspended"     "Active/Verified"
                    │           │              │
...
```
START (Agen, status akun Verified)
  │
  ▼
...
7. Pilihan framework frontend yang mendukung SSR/SSG (lihat SEO & Analytics Specification Bagian 1.1) dan siapa pemegang akun organisasi untuk Google Search Console/GTM/GA4 — perlu diputuskan sebelum development Modul 11 dimulai, idealnya di awal Fase 1.
