...
**Versi:** 1.1 (selaras dengan PRD, ERD & Skema Database, ERD Diagram, API Specification, User Flow, dan SEO & Analytics Specification — seluruhnya v1.1, 26 Juli 2026)
**Disusun oleh:** Principal Software Architect (berdasarkan review menyeluruh dokumen sumber v1.0, kemudian diperbarui setelah audit konflik lintas dokumen diperbaiki langsung di seluruh dokumen sumber pada revisi v1.1).
...
> Framework ini adalah **keputusan arsitektur final** (lihat "Riwayat Keputusan Arsitektur" di atas, poin #6) — sudah diterapkan konsisten di seluruh dokumen sumber v1.1, dan menjadi acuan wajib bagi AI Coding Assistant.
...
| **Backend/API** | Node.js (TypeScript) — **Next.js Route Handlers** untuk BFF ringan, atau service backend terpisah (NestJS/Express TypeScript) jika kompleksitas modul (DBR, RBAC, SEO Indexing API) membutuhkan pemisahan proses dari frontend. | API Spec mendefinisikan REST murni dengan JWT — kompatibel dengan kedua pola; tim wajib memilih salah satu secara eksplisit sebelum Fase 1 selesai, **tidak boleh mencampur** keduanya untuk domain yang sama. |
...
| **Search Engine** | Typesense (direkomendasikan API Spec §3) atau Elasticsearch | Untuk `/properties/search`, `/properties/autocomplete`, typo-tolerance & performa filter kombinasi. |
| **Storage/CDN** | Supabase Storage / Cloudinary / ImageKit (buckets terpisah: publik untuk foto listing, privat+terenkripsi untuk dokumen legalitas) | Sesuai API Spec §9.2. |
| **Cache/Rate Limit** | Redis | Untuk rate limiting (API Spec §0.5), cache halaman publik edge-level (TTFB target SEO Spec §5), dan session refresh token blocklist. |
| **Job Queue** | Redis-based queue (BullMQ) atau Supabase Edge Functions + cron | Untuk: regenerasi sitemap otomatis, kalkulasi ulang counter denormalisasi (`total_listings_sold`, `cta_click_count`), pengiriman notifikasi, panggilan Google Indexing API. |
| **Maps/Geocoding** | Google Maps Platform atau Mapbox (belum final — API Spec §13 poin 2) | Autocomplete alamat client-side; reverse geocoding & distance matrix server-side (API key rahasia). |
| **Analytics** | GTM + GA4, dikonfigurasi via `system_configs`, dikelola Superadmin saja | Sesuai SEO Spec §4. |
| **CI/CD** | Git-based pipeline (GitHub Actions/GitLab CI) — lint, type-check, test, migration check wajib lolos sebelum merge ke `main` | Bagian 15 (Development Rules). |
...
/apps
  /web                       # Next.js app (publik + dashboard + admin, dipisah via route groups)
    /app
      ...
      /(admin)/               # CSR privat — admin panel, noindex, role-gated
        ...
        /rbac/
        /system-config/
      /api/                   # Route handlers BFF tipis (opsional, jika tidak pakai service backend terpisah)
    /components
  ...
  /api                        # Backend service terpisah (jika dipilih arsitektur split — lihat Bagian 4)
    /src
      /modules
        /auth/
        /agents/
      ...
      /shared/
        ...
        /logger/
        /validators/
      /jobs/                  # cron & queue consumers (sitemap regen, counter sync, reminders)
    /migrations                # SQL migration murni, sinkron dengan ERD
...
- `packages/shared-types` adalah **satu-satunya** tempat definisi tipe entitas (Listing, AgentProfile, dsb.) — dilarang mendefinisikan ulang shape data yang sama di `/apps/web` dan `/apps/api` secara terpisah.
...
*Dokumen ini disusun berdasarkan review menyeluruh terhadap: PRD-Real-Estate-Agency-Platform-v1.1.md, ERD-Skema-Database-Real-Estate-Agency-v1.1.md, ERD-Diagram-v1.1.mermaid, API-Specification-Real-Estate-Agency-Platform-v1.1.md, User-Flow-Real-Estate-Agency-Platform-v1.1.md, dan SEO-Analytics-Specification-Real-Estate-Agency-Platform-v1.1.md (seluruhnya v1.1, per 26 Juli 2026 — hasil resolusi audit konflik terhadap dokumen v1.0). Menjadi acuan tetap bagi seluruh AI Coding Assistant dan kontributor manusia selama proyek berjalan.*
