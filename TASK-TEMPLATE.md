...
**Dokumen acuan:** `PROJECT-CONSTITUTION.md`, dokumen sumber v1.1 (PRD/ERD/API Spec/User Flow/SEO Spec), `SYSTEM-ARCHITECTURE.md`, `technology-decisions.md`, `dependency-manifest.md`, `AI-DEVELOPMENT-BLUEPRINT.md` (acuan aktif), `AI-CONTEXT-PACK.md`, `DEVELOPMENT-ROADMAP.md`.

> **Kedudukan dokumen:** Template ini adalah **wadah operasional** — ia tidak membuat keputusan arsitektur/bisnis baru, hanya menstandardisasi cara sebuah task ditulis, diterima, dikerjakan, dan dinyatakan selesai. Jika isi sebuah field bertentangan dengan dokumen governance manapun, dokumen governance yang menang (lihat `AI-DEVELOPMENT-BLUEPRINT.md` Bagian 1 — hierarki dokumen).
...
5. Task ini kemudian diserahkan ke AI Coding Assistant/developer mengikuti **AI Workflow** (`AI-DEVELOPMENT-BLUEPRINT.md` Bagian 4) — task yang tidak lengkap sesuai template ini **wajib ditolak/dikembalikan** untuk dilengkapi dulu, bukan dikerjakan dengan asumsi.
6. Field ini menyempurnakan (bukan menggantikan) 5 elemen wajib **AI Prompting Rules** (`AI-DEVELOPMENT-BLUEPRINT.md` Bagian 21: Module, Scope, Acceptance Criteria, Reference Documents, Out of Scope) — pemetaannya: `Business Goal`+`Background` = Module & konteks, `Problem Statement`+`Files to Modify` = Scope, `Acceptance Criteria` = Acceptance Criteria, `Dependencies` = Reference Documents, `Files Not Allowed to Modify` = Out of Scope.
...
| 2 | **New Module** | Dependencies lintas modul (lihat Module Dependency Blueprint) & Database Impact besar |
...
## Project
<Nama proyek/area — mis. "Platform Web Real Estate Agency — Modul 3 Listing">
...
Jika bagian dokumen sumber terkait belum ada sebagai file (lihat AI-DEVELOPMENT-BLUEPRINT.md Bagian 5),
...
(rujuk Module Dependency & Feature Dependency di AI-DEVELOPMENT-BLUEPRINT.md, dan kolom "Prasyarat" di
DEVELOPMENT-ROADMAP.md). Sertakan juga dokumen sumber spesifik yang WAJIB dirujuk selama pengerjaan
(setara "Reference Documents" — AI-DEVELOPMENT-BLUEPRINT.md Bagian 21), mis.:
- PRD Modul 3.2 (field wajib listing)
- ERD tabel `listings`, `listing_photos`
- API Specification Bagian 2
Jika tidak ada dependency: "Tidak ada — task ini independen dari modul/sprint lain".>

## Database Impact
<Jelaskan eksplisit, jangan diasumsikan:
- Tabel/kolom yang ditambah/diubah/dihapus (nama persis, sesuai ERD).
- Migration baru diperlukan? (Ya/Tidak — jika Ya, nomor/nama file migration).
- Index baru diperlukan? Constraint baru?
- Perlu sinkronisasi ERD-Skema-Database & ERD-Diagram.mermaid? (Ya/Tidak)
- Dampak ke RLS policy (Supabase)?
- Data sensitif tersentuh? (dokumen legalitas, field finansial DBR) — jika ya, tegaskan aturan enkripsi
  at-rest tetap berlaku.
Jika tidak ada dampak database: "Tidak ada — task ini tidak menyentuh skema/data.">

## API Impact
<Jelaskan eksplisit:
- Endpoint baru/diubah/di-deprecate (method + path persis, sesuai API-Specification-v1.1.md).
...
- Perlu sinkronisasi API-Specification-v1.1.md? (Ya/Tidak)
...
- Halaman/route group yang tersentuh ((public)/(auth)/(dashboard)/(admin)) dan strategi rendering-nya
  (SSR/SSG/ISR/CSR — lihat AI-DEVELOPMENT-BLUEPRINT.md Bagian 9 pola Next.js jika berlaku ke Blueprint aktif).
...
- Dampak ke state management (server state/UI state) sesuai keputusan resmi yang berlaku.
Jika tidak ada dampak frontend: "Tidak ada — task ini murni backend/database/infrastruktur.">

## Backend Impact
<Jelaskan eksplisit:
- Modul/service/repository yang tersentuh (`*.controller.ts`/`*.service.ts`/`*.repository.ts`).
- Middleware yang tersentuh (auth/rbac/rate-limit).
- Job/queue/trigger yang tersentuh (sitemap regen, counter sync, notifikasi).
- Dampak ke business logic sensitif (kalkulasi DBR, resolusi ownership/RBAC).
...
"Out of Scope" (AI-DEVELOPMENT-BLUEPRINT.md Bagian 21). Termasuk modul lain yang tidak terkait,
skema tabel di luar yang disebut di Database Impact, dan dokumen sumber "BERLAKU"/final
(PROJECT-CONSTITUTION.md, PRD/ERD/API Spec v1.1) yang tidak boleh diubah keputusannya tanpa
instruksi eksplisit manusia — hanya boleh disinkronkan sebagai detail baru yang konsisten.>
...
## Testing Checklist
<Mengikuti Testing Rules yang berlaku (Vitest/RTL/Playwright bila sudah dikonfirmasi sebagai
keputusan resmi yang berlaku untuk proyek ini — lihat technology-decisions.md status terkini):
...
- Feature flag/toggle tersedia untuk menonaktifkan tanpa revert penuh? (jika ada)
...
Definition of Done yang berlaku di Blueprint aktif):
...
- [ ] Tidak ada pelanggaran Architecture Constraints yang berlaku
...
(rujuk AI Workflow & Documentation Reading Order di Blueprint aktif):
...
- [ ] Sudah memverifikasi seluruh Dependencies di atas benar-benar sudah selesai/tersedia
...
4. **Perubahan pada dokumen governance final (Constitution, PRD/ERD/API Spec v1.1) tidak pernah menjadi "Files to Modify"** dalam task teknis biasa — perubahan semacam itu adalah task governance tersendiri yang memerlukan persetujuan eksplisit manusia.
...
*Template ini adalah alat operasional turunan dari `AI-DEVELOPMENT-BLUEPRINT.md` (versi aktif) dan seluruh dokumen governance proyek. Dipakai berulang untuk setiap task development apa pun jenisnya, disimpan satu file per task di `/docs/tasks/`, dan menjadi unit kerja standar yang dapat diaudit sepanjang siklus proyek.*
