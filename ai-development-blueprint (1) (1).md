...
| **Last Updated** | 28 Juli 2026 — disinkronkan terhadap `architecture-decision-records.md` (ADR terbaru, khususnya **ADR-001 Backend Architecture — Approved 27 Juli 2026**) |
...
>
> **Catatan ADR (wajib dibaca berdampingan dengan catatan di atas):** `architecture-decision-records.md` **wajib dibaca sebelum** `technology-decisions.md` (ADR Bagian 10 poin 1) karena ADR menjelaskan alasan & alternatif di balik setiap baris *Official Technology Stack*. ADR berstatus **Approved mengikat** implementasi (termasuk seluruh isi Blueprint ini); ADR berstatus **OPEN** (saat ini: ADR-005 Search Strategy, ADR-006 Job Queue Strategy, ADR-008 Maps Provider, ADR-018 Caching Strategy) **tidak boleh diasumsikan** oleh AI Coding Assistant — area yang terdampak wajib diimplementasikan sebagai *configurable placeholder* dengan `// TODO: menunggu resolusi ADR-XXX` (lihat Bagian 4 & 24).
...
- **Langkah 1 (Read ADR) adalah langkah baru dan wajib paling awal** — mengikuti hierarki governance di Bagian 1: ADR berstatus **Approved** (termasuk **ADR-001 Backend Architecture**: Next.js Route Handlers + Supabase, tanpa service terpisah, dengan Bolt.new sebagai bagian toolchain resmi) mengikat seluruh langkah implementasi berikutnya dan tidak boleh diperdebatkan ulang oleh AI. ADR berstatus **OPEN** (ADR-005, ADR-006, ADR-008, ADR-018) menandai area yang **belum boleh diasumsikan** — jika task menyentuh area ini, AI wajib mengikuti gerbang Langkah 6→7.
...
3. Task yang menyentuh item berstatus "Hal Perlu Dikonfirmasi"/Open Question, **termasuk ADR berstatus OPEN** (saat ini: ADR-005 Search Strategy, ADR-006 Job Queue Strategy, ADR-008 Maps Provider, ADR-018 Caching Strategy), **wajib** diimplementasikan sebagai *configurable placeholder*, ditandai `// TODO: menunggu resolusi ADR-XXX` — AI tidak mengambil keputusan arsitektur/bisnis tsb sendiri meski task memungkinkan, dan tidak berwenang menaikkan status ADR menjadi Approved (ADR Bagian 9).
...
Panduan operasional untuk membangun modul secara individu, turunan langsung dari **ADR-001 (Backend Architecture, Approved)** dan `DEVELOPMENT-ROADMAP.md` (Module Order & Dependency Graph). Bagian ini menjawab pertanyaan *"bagaimana satu modul dibangun dari nol sampai selesai"* — berbeda dari Bagian 23 (Development Order) yang menjawab *"modul mana dulu"*.
...
Empat ADR berstatus OPEN saat ini berdampak langsung pada modul tertentu (lihat ADR Bagian 7 — Impact Analysis):

| ADR OPEN | Modul Terdampak | Cara Menangani Saat Development Modul |
|---|---|---|
| **ADR-005** Search Strategy | Modul 3 (search/filter/autocomplete lanjutan) | Bangun CRUD listing dasar & filter sederhana (query Postgres langsung) terlebih dahulu — search lanjutan (full-text/Elasticsearch/Algolia) ditandai `// TODO: menunggu resolusi ADR-005`, tidak memblokir Sprint S0–S4. |
...
Modul yang **tidak** tersentuh keempat ADR di atas (M1, M2 dasar, M4, M7, M9/M10, M2 ext.) dapat dibangun penuh tanpa placeholder terkait ADR.
...
| 4 | Modul 3 — Listing Management | #2, #3, Region Data | Entitas inti platform. **Catatan ADR:** CRUD & filter dasar tidak diblokir; search lanjutan menunggu ADR-005, lokasi peta menunggu ADR-008 (lihat Bagian 22.3). |
...
| 2 (High) | **ADR-005** — Search Strategy | Sprint S5 (search lanjutan), bagian dari #4 di atas | Dapat diselesaikan paralel dengan ADR-006. |
| 3 (Medium) | **ADR-008** — Maps Provider | Sprint S4 (#4) dan S9 (#8) | Independen, dapat diputuskan kapan pun — murni menunggu konfirmasi biaya bisnis. |
| 4 (Low) | **ADR-018** — Caching Strategy | Tidak memblokir sprint manapun di roadmap saat ini | Direkomendasikan diputuskan bersamaan ADR-006 jika arah Job Queue mengarah ke BullMQ (butuh Redis). |

**Catatan penting:** ADR-001 (Backend Architecture) **telah selesai** (Approved, 27 Juli 2026) dan tidak lagi menjadi item dalam tabel resolusi ini — seluruh Development Order di atas kini dapat dieksekusi dengan konteks arsitektur backend yang sudah pasti (Route Handlers + Supabase, satu `apps/web`, Bolt.new sebagai bagian toolchain resmi).
...
Aturan emas berikut wajib dipatuhi seluruh AI Coding Assistant (Claude, Bolt.new, ChatGPT, Cursor, GitHub Copilot) tanpa pengecualian, sepanjang lifecycle proyek. **Poin 1–3 diperbarui di v1.1**; poin 32–34 baru ditambahkan untuk mengintegrasikan ADR terbaru.
...
*Dokumen ini adalah turunan operasional dari `architecture-decision-records.md`, `PROJECT-CONSTITUTION.md`, dan seluruh dokumen sumber proyek v1.1 (26–28 Juli 2026), berpasangan erat dengan `technology-decisions.md`, `dependency-manifest.md`, dan `DEVELOPMENT-ROADMAP.md`. Wajib direview ulang setiap kali ada ADR baru yang disahkan atau perubahan signifikan pada dokumen governance di atasnya (lihat Bagian 1 — hierarki dokumen), dan menjadi acuan tetap bagi seluruh AI Coding Assistant serta kontributor manusia selama lifecycle proyek berlangsung.*
