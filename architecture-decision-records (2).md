...
**Dependencies:** Bergantung pada ADR-021 (Frontend Framework — Approved: Next.js App Router). Menjadi prasyarat bagi ADR-005 (Search Strategy, **kini Approved** — lihat Bagian 4), ADR-006 (Job Queue Strategy, masih OPEN — lihat opsi Hybrid di atas), ADR-012 (API Architecture, sudah Approved secara independen — kini lokasi eksekusinya terkonfirmasi konsisten), ADR-018 (Caching Strategy, masih OPEN).
...
**Dependencies:** Bergantung pada ADR-001 (Backend Architecture — Approved: Route Handlers + Supabase). Tidak lagi memblokir/diblokir oleh ADR-006 (Job Queue Strategy, masih OPEN) untuk Fase 1 — ketergantungan pada ADR-006 baru relevan saat migrasi Fase 2 ke Typesense dieksekusi.
...
**Dependencies:** Sangat terkait **ADR-001** (Backend Architecture — **kini Approved**: Route Handlers + Supabase) — dengan backend terkunci ke Route Handlers+Supabase, opsi Edge Functions+cron kini secara arsitektural adalah pilihan yang paling konsisten untuk dipertimbangkan lebih dulu.

**Review Date:** Wajib diselesaikan **sebelum Sprint S6/S13**.

**Notes:** ADR-001 yang menaungi keputusan ini telah diselesaikan (Approved, 27 Juli 2026) — keputusan ADR-006 kini dapat diambil dengan konteks arsitektur backend yang sudah pasti (Route Handlers + Supabase), memperkuat arah Edge Functions+cron dibanding sebelumnya saat kedua ADR masih sama-sama Open.
...
|---|---|---|---|---|---|
| **ADR-006** Job Queue Strategy | OPEN | **High** | Tidak masuk *Official Technology Stack*; ADR-001 yang menaunginya kini sudah Approved — arah Edge Functions+cron menguat, namun keputusan final tetap menunggu pengesahan eksplisit | technology-decisions.md §9.4, SEO Spec §1.5/§4.3 | Sebelum Sprint S6/S13 |
...
1. **ADR-006 — Job Queue Strategy** — kini prioritas tertinggi berikutnya untuk diselesaikan, karena arah Edge Functions+cron menguat secara arsitektural setelah ADR-001 dikunci ke Route Handlers+Supabase.
2. **ADR-008 — Maps Provider** — sepenuhnya independen, dapat diselesaikan **kapan pun secara paralel** dengan #1, murni menunggu konfirmasi biaya dari tim bisnis.
3. **ADR-018 — Caching Strategy** — prioritas terendah, direkomendasikan diputuskan **setelah** ADR-006 (agar keputusan Redis, jika ada, diambil satu kali untuk dua kebutuhan sekaligus).
