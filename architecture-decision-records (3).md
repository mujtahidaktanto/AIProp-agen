...
- **Hybrid: Route Handlers + Supabase Edge Functions untuk logic berat/sensitif** *(tidak ditolak, dipisah ke ADR-006)*: skor tertinggi pada kesesuaian PRD/ERD, namun sengaja tidak dijadikan bagian keputusan ADR-001 agar tidak mencampur dua keputusan independen — cakupan job asinkron/webhook menjadi bagian ADR-006 (Job Queue Strategy, **kini Approved**: Vercel Cron Jobs + Postgres Trigger/Database Webhook Fase 1, bukan Edge Functions — lihat Bagian 4). ADR-001 tetap kompatibel penuh dengan pendekatan Edge Functions di masa depan tanpa perlu direvisi, bila kebutuhan berubah.

**Consequences:** Proyek terikat pada batas eksekusi fungsi serverless Vercel (~10–60 detik tergantung paket) — proses berat di masa depan (bulk processing, batch job) wajib diarahkan ke mekanisme Job Queue (ADR-006, **kini Approved**: Vercel Cron Jobs + Postgres Trigger/Database Webhook, migrasi terjadwal ke QStash di Fase 2), bukan dipaksakan ke Route Handler; hal ini perlu didokumentasikan eksplisit sebagai constraint arsitektur, bukan asumsi tersembunyi. Migrasi ke service terpisah di masa depan (bila kebutuhan skalabilitas berubah signifikan, dikonfirmasi data produksi) memerlukan ekstraksi logic dari Route Handlers — dapat dilakukan bertahap karena logic tetap TypeScript murni (ADR-025), namun tetap merupakan pekerjaan migrasi non-trivial. Konvensi API (ADR-012, bentuk kontrak) tidak berubah — hanya lokasi eksekusinya yang kini terkunci.
...
**Dependencies:** Bergantung pada ADR-021 (Frontend Framework — Approved: Next.js App Router). Menjadi prasyarat bagi ADR-005 (Search Strategy, **kini Approved** — lihat Bagian 4), ADR-006 (Job Queue Strategy, **kini Approved** — lihat Bagian 4), ADR-012 (API Architecture, sudah Approved secara independen — kini lokasi eksekusinya terkonfirmasi konsisten), ADR-018 (Caching Strategy, masih OPEN).
...
**Affected Documents:** `technology-decisions.md` §9.2, `API-Specification-v1.1.md` §3, `dependency-manifest.md`, `SEO-Analytics-Specification-v1.1.md` (indexing sinkron), `ERD-Skema-Database-Real-Estate-Agency-Platform-v1.1.md` (kolom `search_vector`), `ai-development-blueprint.md` §22.3/§23, `decision-log.md`, `CHANGELOG.md`, `CURRENT-PROJECT-STATE.md`.
...
**Affected Documents:** `technology-decisions.md` §9, `SEO-Analytics-Specification-v1.1.md` §1.5 & §4.3, `dependency-manifest.md`, `SYSTEM-ARCHITECTURE.md`, `PROJECT-CONSTITUTION.md`, `development-playbook.md`, `decision-log.md`, `CHANGELOG.md`, `CURRENT-PROJECT-STATE.md`, `ERD-Skema-Database-Real-Estate-Agency-Platform-v1.1.md` (trigger counter sync, tabel `job_execution_log`).

**Dependencies:** Bergantung pada ADR-001 (Backend Architecture — Approved: Route Handlers + Supabase) dan ADR-010 (Deployment Strategy — Approved: Vercel, menyediakan fitur Cron Jobs). Tidak lagi memblokir/diblokir oleh ADR-018 (Caching Strategy, masih OPEN) — keputusan Redis untuk ADR-018 tidak diperlukan oleh mekanisme job queue Fase 1 ini.
...
**Pros/Cons:** *(Redis)* Pro: cache granular & rate-limit terpusat yang presisi; Con: menambah komponen infrastruktur baru — **kini sepenuhnya keputusan independen**, karena ADR-006 (Job Queue) telah diselesaikan **tanpa** BullMQ/Redis (Approved: Vercel Cron Jobs + Postgres Trigger, migrasi Fase 2 ke QStash — juga tidak butuh Redis). *(Tanpa cache tambahan)* Pro: kesederhanaan operasional; Con: rate limiting endpoint sensitif saat ini belum punya mekanisme penyimpanan status lintas-instance yang eksplisit selain "blocklist Redis/DB" yang disebut di `PROJECT-CONSTITUTION.md` §10 tanpa kepastian implementasi konkret.
...
**Dependencies:** Sebelumnya terkait ADR-006 (Job Queue) — kini **tidak lagi bergantung**, karena ADR-006 (Approved, 29 Juli 2026) tidak memilih BullMQ/Redis sebagai mekanisme job queue (memilih Vercel Cron Jobs + Postgres Trigger, migrasi Fase 2 ke QStash). Keputusan Redis untuk ADR-018, jika diperlukan, kini murni berdiri sendiri untuk kebutuhan cache/rate-limiting.

**Review Date:** Dapat ditunda melewati MVP awal — tidak lagi perlu dipertimbangkan bersamaan ADR-006 (sudah resolved tanpa Redis); dievaluasi tersendiri saat kebutuhan rate limiting lintas-instance menjadi mendesak.
...
| **ADR-018** Caching Strategy (level aplikasi/Redis) | OPEN | **Low** | Hanya tercatat sebagai *Future Decision* Proposed, belum ada kebutuhan aktif mendesak; sebelumnya terkait ADR-006, kini sepenuhnya independen karena ADR-006 resolved tanpa Redis | technology-decisions.md, decision-log.md Bagian 10 | Dapat ditunda melewati MVP, dievaluasi tersendiri |
...
ADR-012 (API Architecture)
   ↓
ADR-006 (Job Queue Strategy) [APPROVED]      ADR-018 (Caching/Redis) [OPEN]
   (tidak lagi saling terkait — ADR-006 resolved tanpa Redis)
   ↓
ADR-020 (Notification Strategy)
```
...
**Simpul kritis:** **ADR-001** adalah simpul dengan dependency turunan terbanyak (ADR-005, ADR-006, ADR-012, ADR-018, dan transitif ADR-013/014/015/016/020) — ini mengonfirmasi penilaian `executive-architecture-review.md` bahwa ADR-001 adalah prioritas Critical tunggal. **ADR-001 kini berstatus Approved** (27 Juli 2026, via Architecture Review Board) — seluruh ADR turunan yang bergantung padanya kini dapat dinilai ulang dengan konteks backend yang sudah pasti. **ADR-005 kini juga berstatus Approved** (28 Juli 2026) dan **ADR-006 kini juga berstatus Approved** (29 Juli 2026, via Architecture Review Board) — hanya **ADR-008** dan **ADR-018** yang tetap OPEN dan memerlukan keputusan tersendiri; keduanya kini sepenuhnya independen satu sama lain (ADR-018 tidak lagi terkait ADR-006 setelah ADR-006 resolved tanpa Redis).
...
>
> **ADR-005 (Search Strategy) dihapus dari daftar OPEN di bagian ini** — telah diselesaikan (Approved, 28 Juli 2026, via Architecture Review Board: PostgreSQL Full-Text Search + pg_trgm Fase 1, migrasi terjadwal ke Typesense Fase 2). Lihat ADR-005 di Bagian 4 untuk Context/Decision/Rationale/Consequences lengkap.
>
> **ADR-006 (Job Queue Strategy) dihapus dari daftar OPEN di bagian ini** — telah diselesaikan (Approved, 29 Juli 2026, via Architecture Review Board: Vercel Cron Jobs + Postgres Trigger/Database Webhook Fase 1, migrasi terjadwal ke QStash Fase 2). Lihat ADR-006 di Bagian 4 untuk Context/Decision/Rationale/Consequences lengkap.
...
0. ~~**ADR-005 — Search Strategy**~~ — **SELESAI** (Approved, 28 Juli 2026, via Architecture Review Board). PostgreSQL FTS+pg_trgm Fase 1, migrasi terjadwal ke Typesense Fase 2 berdasarkan kriteria ambang eksplisit.
0. ~~**ADR-006 — Job Queue Strategy**~~ — **SELESAI** (Approved, 29 Juli 2026, via Architecture Review Board). Vercel Cron Jobs + Postgres Trigger/Database Webhook Fase 1, migrasi terjadwal ke QStash Fase 2 berdasarkan kriteria ambang eksplisit. Ditolak: BullMQ+Redis (worker long-running tidak kompatibel dengan model serverless ADR-001).
1. **ADR-008 — Maps Provider** — kini prioritas tertinggi berikutnya untuk diselesaikan (bersama ADR-018, keduanya sepenuhnya independen satu sama lain) — sepenuhnya independen dari ADR lain, dapat diselesaikan **kapan pun**, murni menunggu konfirmasi biaya dari tim bisnis.
2. **ADR-018 — Caching Strategy** — dapat diselesaikan **paralel** dengan ADR-008; tidak lagi perlu menunggu ADR-006 (sudah resolved tanpa Redis) — dievaluasi murni berdasarkan kebutuhan rate limiting lintas-instance.
