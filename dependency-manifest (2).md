...
| **Version** | 1.2 |
| **Status** | Draft — menunggu review & pengesahan tim (belum eligible untuk status Baseline di `document-governance-baseline-register.md` selama ADR-006 Job Queue Strategy, ADR-008 Maps Provider, dan ADR-018 Caching Strategy masih **OPEN** — lihat Bagian 10. Satu Open Decision yang sebelumnya memengaruhi dokumen ini — **ADR-005 Search Strategy** — telah **Approved** dan diintegrasikan penuh ke revisi ini.) |
| **Last Updated** | 28 Juli 2026 — direvisi (v1.1 → v1.2) untuk mengintegrasikan resolusi **ADR-005 (Search Strategy, Approved)** dari `architecture-decision-records.md` dan `technology-decisions.md` v1.2 |
...
| **Phase 2 — Database & Backend** | Koneksi Supabase, migration awal, Route Handlers sebagai BFF (ADR-001), aktivasi ekstensi `pg_trgm` + kolom `search_vector` pada `listings` (ADR-005) | `@supabase/supabase-js`, `@supabase/ssr`, `supabase` (CLI, dev dependency) — `pg_trgm` diaktifkan via migration SQL, bukan package npm |
...
5. **Package berstatus "belum final"** (Maps SDK di Bagian 3, seluruh kandidat Future Evaluation di `technology-decisions.md` Bagian 8) tidak boleh diinstal sebagai keputusan final sepihak — implementasikan sebagai placeholder/interface yang provider-agnostic jika memungkinkan, tandai `// TODO: menunggu resolusi ADR-XXX` (ADR-006/008/018 — lihat Bagian 10). **Pengecualian:** ADR-005 (Search Strategy) sudah **Approved** — `pg_trgm` boleh diaktifkan langsung, dan `typesense` boleh ditambahkan **hanya** setelah kriteria ambang migrasi Fase 2 tercapai (bukan diinstal lebih awal "untuk jaga-jaga").
...
3. **Package Job Queue belum ditentukan** (lihat **ADR-006, OPEN — target sebelum Sprint S6/S13**, `technology-decisions.md` Bagian 9 poin 2) — dengan ADR-001 kini Approved ke arah Route Handlers + Supabase, opsi **Supabase Edge Functions + cron** menguat secara arsitektural dan **tidak memerlukan package npm tambahan** di sisi aplikasi Next.js. Jika keputusan akhir tetap mengarah ke BullMQ, package `bullmq` + `ioredis` perlu ditambahkan (sekaligus menyelesaikan poin 2 di atas jika Redis dipakai bersama). Perlu dicatat: mekanisme sinkronisasi index untuk migrasi Search Engine Fase 2 (`typesense`, lihat Bagian 3) juga bergantung pada resolusi poin ini.
4. **Kriteria ambang migrasi Search Engine ke Typesense** (ADR-005 Approved — volume listing >±50.000, latensi p95 `/properties/search` >500ms, atau keluhan relevansi berulang ≥3 laporan/sprint) perlu dipantau berkala pasca-launch; package `typesense` hanya ditambahkan ke dokumen ini **saat** salah satu kriteria benar-benar tercapai dan migrasi disetujui dieksekusi — bukan diinstal preventif.
