...
| **Version** | 1.3 |
| **Status** | Draft — menunggu review & pengesahan tim (belum eligible untuk status Baseline di `document-governance-baseline-register.md` selama ADR-008 Maps Provider dan ADR-018 Caching Strategy masih **OPEN** — lihat Bagian 10. Dua Open Decision yang sebelumnya memengaruhi dokumen ini — **ADR-005 Search Strategy** dan **ADR-006 Job Queue Strategy** — telah **Approved** dan diintegrasikan penuh ke revisi ini.) |
| **Last Updated** | 29 Juli 2026 — direvisi (v1.2 → v1.3) untuk mengintegrasikan resolusi **ADR-006 (Job Queue Strategy, Approved)** dari `architecture-decision-records.md` dan `technology-decisions.md` v1.3 |
...
5. **Package berstatus "belum final"** (Maps SDK di Bagian 3, seluruh kandidat Future Evaluation di `technology-decisions.md` Bagian 8) tidak boleh diinstal sebagai keputusan final sepihak — implementasikan sebagai placeholder/interface yang provider-agnostic jika memungkinkan, tandai `// TODO: menunggu resolusi ADR-XXX` (ADR-008/018 — lihat Bagian 10). **Pengecualian:** ADR-005 (Search Strategy) dan ADR-006 (Job Queue Strategy) sudah **Approved** — `pg_trgm` boleh diaktifkan langsung, Vercel Cron/Postgres Trigger boleh dikonfigurasi langsung, dan `typesense`/`@upstash/qstash` boleh ditambahkan **hanya** setelah kriteria ambang migrasi Fase 2 masing-masing tercapai (bukan diinstal lebih awal "untuk jaga-jaga"). `bullmq`/`ioredis` **tidak pernah** menjadi kandidat sah untuk kebutuhan job queue proyek ini.
...
| **ADR status watch** | Setiap kali status ADR di `architecture-decision-records.md` berubah (terutama ADR-008/018, saat ini OPEN), dokumen ini wajib direvisi bersamaan — package baru (mis. Maps SDK final) hanya ditambahkan setelah ADR terkait naik status ke **Approved**. **ADR-005 (Search Strategy)** dan **ADR-006 (Job Queue Strategy)** sudah Approved (28 & 29 Juli 2026) — `typesense` dan `@upstash/qstash` tetap dipantau sebagai kandidat Fase 2 masing-masing, ditambahkan hanya saat kriteria ambang migrasi tercapai dan migrasi benar-benar dieksekusi, bukan otomatis mengikuti status Approved ADR-nya. `bullmq`/`ioredis` **tidak pernah** menjadi kandidat sah — dikecualikan permanen dari roadmap dependency job queue proyek ini. |
...
2. **Package Cache/Rate Limiting belum ditentukan** (lihat **ADR-018, OPEN — prioritas terendah, dapat ditunda melewati MVP**, `technology-decisions.md` Bagian 9 poin 1) — jika keputusan mengarah ke Redis, package terkait (mis. `ioredis`, atau SDK rate-limiting edge seperti `@upstash/ratelimit` + `@upstash/redis` jika memakai Redis terkelola serverless) perlu ditambahkan melalui revisi. **Sepenuhnya independen dari ADR-006** — Job Queue sudah resolved tanpa Redis (Vercel Cron + Postgres Trigger, bukan BullMQ), sehingga keputusan Redis di sini kini murni berdiri sendiri untuk kebutuhan cache/rate-limiting.
3. **Kriteria ambang migrasi Search Engine ke Typesense** (ADR-005 Approved — volume listing >±50.000, latensi p95 `/properties/search` >500ms, atau keluhan relevansi berulang ≥3 laporan/sprint) perlu dipantau berkala pasca-launch; package `typesense` hanya ditambahkan ke dokumen ini **saat** salah satu kriteria benar-benar tercapai dan migrasi disetujui dieksekusi — bukan diinstal preventif.
4. **Kriteria ambang migrasi Job Queue ke QStash** (ADR-006 Approved — volume job harian melampaui kapasitas batching per invocation, kebutuhan retry/backoff/dead-letter kompleks, atau frekuensi melampaui batas cron interval tier Vercel) perlu dipantau berkala pasca-launch; package `@upstash/qstash` hanya ditambahkan ke dokumen ini **saat** salah satu kriteria benar-benar tercapai dan migrasi disetujui dieksekusi — bukan diinstal preventif.
