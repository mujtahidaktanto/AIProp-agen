...
| **Last Updated** | 29 Juli 2026 — disinkronkan terhadap `architecture-decision-records.md` (ADR terbaru, khususnya **ADR-001 Backend Architecture — Approved 27 Juli 2026**, **ADR-005 Search Strategy — Approved 28 Juli 2026**, dan **ADR-006 Job Queue Strategy — Approved 29 Juli 2026**) |
...
| **Related Documents** | `architecture-decision-records.md` (ADR), `PROJECT-CONSTITUTION.md`, `SYSTEM-ARCHITECTURE.md`, `technology-decisions.md`, `dependency-manifest.md`, `AI-CONTEXT-PACK.md`, `PRD-Real-Estate-Agency-Platform-v1.1.md`, `ERD-Skema-Database-Real-Estate-Agency-v1.1.md`, `ERD-Diagram-v1.1.mermaid`, `API-Specification-Real-Estate-Agency-Platform-v1.1.md`, `User-Flow-Real-Estate-Agency-Platform-v1.1.md`, `SEO-Analytics-Specification-Real-Estate-Agency-Platform-v1.1.md`, `DEVELOPMENT-ROADMAP.md` |
...
architecture-decision-records.md (ADR, khusus entri berstatus Approved)
   ...
   >  AI-DEVELOPMENT-BLUEPRINT.md (dokumen ini)
```
...
>
> **Catatan ADR (wajib dibaca berdampingan dengan catatan di atas):** `architecture-decision-records.md` **wajib dibaca sebelum** `technology-decisions.md` (ADR Bagian 10 poin 1) karena ADR menjelaskan alasan & alternatif di balik setiap baris *Official Technology Stack*. ADR berstatus **Approved mengikat** implementasi (termasuk seluruh isi Blueprint ini); ADR berstatus **OPEN** (saat ini: ADR-008 Maps Provider, ADR-018 Caching Strategy) **tidak boleh diasumsikan** oleh AI Coding Assistant — area yang terdampak wajib diimplementasikan sebagai *configurable placeholder* dengan `// TODO: menunggu resolusi ADR-XXX` (lihat Bagian 4 & 24). **ADR-005 (Search Strategy) dan ADR-006 (Job Queue Strategy) sudah Approved** — search Fase 1 dan job asinkron/terjadwal boleh diimplementasikan penuh, lihat Bagian 22.3.
...
- **Langkah 1 (Read ADR) adalah langkah baru dan wajib paling awal** — mengikuti hierarki governance di Bagian 1: ADR berstatus **Approved** (termasuk **ADR-001 Backend Architecture**: Next.js Route Handlers + Supabase, tanpa service terpisah, dengan Bolt.new sebagai bagian toolchain resmi; **ADR-005 Search Strategy**: PostgreSQL FTS + `pg_trgm` Fase 1, migrasi terjadwal ke Typesense Fase 2; dan **ADR-006 Job Queue Strategy**: Vercel Cron Jobs + Postgres Trigger/Database Webhook Fase 1, migrasi terjadwal ke QStash Fase 2) mengikat seluruh langkah implementasi berikutnya dan tidak boleh diperdebatkan ulang oleh AI. ADR berstatus **OPEN** (ADR-008, ADR-018) menandai area yang **belum boleh diasumsikan** — jika task menyentuh area ini, AI wajib mengikuti gerbang Langkah 6→7.
...
Struktur folder berikut adalah **keputusan final** (`PROJECT-CONSTITUTION.md` Bagian 5, `SYSTEM-ARCHITECTURE.md` Bagian 6, dikunci oleh **ADR-001 — Backend Architecture, Approved**) — AI **dilarang** membuat struktur paralel atau menyimpang tanpa persetujuan eksplisit. Satu aplikasi (`apps/web`) — **tidak ada** `apps/api` atau service backend Node.js terpisah di cakupan proyek saat ini.
...
| **ADR Status Check** *(baru)* | Daftar ADR yang relevan dengan scope task ini beserta statusnya (Approved/OPEN) — wajib diisi sebelum implementasi, bukan diasumsikan | `ADR-001 (Approved) berlaku untuk lokasi endpoint; ADR-008 Maps Provider (OPEN) — form lokasi listing wajib pakai placeholder configurable, bukan hard-code provider` |
...
2. Jika instruksi task **bertentangan** dengan *hard rule* di `PROJECT-CONSTITUTION.md` (khususnya Bagian 3.2, 11, 20) **atau** dengan ADR berstatus **Approved**, AI **wajib menanyakan konfirmasi eksplisit** sebelum menyimpang — sesuai aturan utama di halaman pembuka Constitution dan ADR Bagian 10 poin 2–3. ADR Approved menang atas dokumen turunan mana pun (`SYSTEM-ARCHITECTURE.md`, `technology-decisions.md`, Blueprint ini) jika terjadi konflik teks.
3. Task yang menyentuh item berstatus "Hal Perlu Dikonfirmasi"/Open Question, **termasuk ADR berstatus OPEN** (saat ini: ADR-008 Maps Provider, ADR-018 Caching Strategy), **wajib** diimplementasikan sebagai *configurable placeholder*, ditandai `// TODO: menunggu resolusi ADR-XXX` — AI tidak mengambil keputusan arsitektur/bisnis tsb sendiri meski task memungkinkan, dan tidak berwenang menaikkan status ADR menjadi Approved (ADR Bagian 9). **ADR-005 (Search Strategy) dan ADR-006 (Job Queue Strategy) sudah Approved** — task pencarian listing Fase 1 dan task job asinkron/terjadwal Fase 1 tidak lagi memerlukan placeholder, lihat poin 6 & 7.
...
Panduan operasional untuk membangun modul secara individu, turunan langsung dari **ADR-001 (Backend Architecture, Approved)**, **ADR-005 (Search Strategy, Approved)**, **ADR-006 (Job Queue Strategy, Approved)**, dan `DEVELOPMENT-ROADMAP.md` (Module Order & Dependency Graph). Bagian ini menjawab pertanyaan *"bagaimana satu modul dibangun dari nol sampai selesai"* — berbeda dari Bagian 23 (Development Order) yang menjawab *"modul mana dulu"*.
...
Dua ADR berstatus OPEN saat ini berdampak langsung pada modul tertentu (lihat ADR Bagian 7 — Impact Analysis):

| ADR OPEN | Modul Terdampak | Cara Menangani Saat Development Modul |
|---|---|---|
| **ADR-008** Maps Provider | Modul 3 (lokasi listing), Modul 6 (peta proyek developer) | Field lokasi cascading (`province_id/city_id/district_id`) tetap dibangun penuh (tidak bergantung provider Maps); komponen autocomplete/peta dibungkus adapter yang provider-nya configurable, ditandai `// TODO: menunggu resolusi ADR-008`. |
| **ADR-018** Caching Strategy | Lintas modul (rate limiting Auth) | Gunakan mekanisme in-memory/DB sederhana untuk rate limiting sementara; jangan mengasumsikan Redis tersedia di kode modul manapun sebelum ADR-018 disahkan. |

> **ADR-005 (Search Strategy) telah diselesaikan** (Approved, 28 Juli 2026) — dihapus dari tabel di atas. Modul 3 (search/filter/autocomplete) kini dibangun **penuh** menggunakan PostgreSQL Full-Text Search + `pg_trgm`, **tidak lagi** memerlukan placeholder `// TODO: menunggu resolusi ADR-005`. Migrasi Fase 2 (Typesense) hanya relevan saat kriteria ambang migrasi tercapai (lihat Bagian 23.2) — AI **tidak** perlu membungkus search sebagai adapter provider-agnostic seperti pola Maps (ADR-008); kontrak `listings.repository.ts` yang sudah ada cukup sebagai titik ekstraksi jika migrasi Fase 2 benar-benar terjadi.
>
...
Modul yang **tidak** tersentuh kedua ADR di atas (M1, M2 dasar, M3 search/filter dasar & lanjutan, M4, M5 reminder H-1, M7, M8 counter sync, M9/M10, M11 sitemap, M2 ext.) dapat dibangun penuh tanpa placeholder terkait ADR.
...
| 1 | Modul 1 — Authentication | #0 | Sumber identitas & `role_id` bagi seluruh entitas ber-ownership. |
...
| 4 | Modul 3 — Listing Management | #2, #3, Region Data | Entitas inti platform. **Catatan ADR:** CRUD, filter, dan search/autocomplete (PostgreSQL FTS + `pg_trgm`, ADR-005 Approved) dibangun penuh tanpa placeholder; lokasi peta menunggu ADR-008 (lihat Bagian 22.3). |
...
| 8 | Modul 6 — Developer Directory | #3, #4, Region Data | Memperluas skema Listing — butuh Listing yang sudah stabil. |
...
**Aturan lintas fase (non-negotiable, `PROJECT-CONSTITUTION.md` Bagian 21 poin 1):** Fase 2 (#8–#9) dan Fase 3 (#10–#11) tidak boleh dimulai sebelum seluruh Acceptance Criteria & Module Completion Checklist (Bagian 24) Fase 1 (#0–#7) lolos penuh.

### 23.2 Urutan Resolusi ADR OPEN (Paralel dengan Development Order di Atas)

Berbeda dari urutan modul di atas, urutan berikut adalah urutan **pengambilan keputusan manusia** atas ADR yang masih OPEN (ADR Bagian 8, Implementation Order) — dijalankan **paralel**, bukan menggantikan, urutan modul 23.1:

| Prioritas | ADR | Wajib Selesai Sebelum | Catatan |
|---|---|---|---|
| 1 (Medium) | **ADR-008** — Maps Provider | Sprint S4 (#4) dan S9 (#8) | Independen, dapat diputuskan kapan pun — murni menunggu konfirmasi biaya bisnis. |
| 2 (Low) | **ADR-018** — Caching Strategy | Tidak memblokir sprint manapun di roadmap saat ini | Sepenuhnya independen — ADR-006 sudah resolved tanpa Redis (Vercel Cron + Postgres Trigger, bukan BullMQ), dievaluasi tersendiri untuk kebutuhan rate limiting. |

**Catatan penting:** ADR-001 (Backend Architecture) **telah selesai** (Approved, 27 Juli 2026), ADR-005 (Search Strategy) **telah selesai** (Approved, 28 Juli 2026), dan ADR-006 (Job Queue Strategy) **telah selesai** (Approved, 29 Juli 2026) — ketiganya tidak lagi menjadi item dalam tabel resolusi ini. Seluruh Development Order di atas kini dapat dieksekusi dengan konteks arsitektur backend, search, dan job queue yang sudah pasti (Route Handlers + Supabase satu `apps/web`; PostgreSQL FTS + `pg_trgm` Fase 1 dengan migrasi terjadwal ke Typesense Fase 2; Vercel Cron Jobs + Postgres Trigger/Database Webhook Fase 1 dengan migrasi terjadwal ke QStash Fase 2; Bolt.new sebagai bagian toolchain resmi).
...
Aturan emas berikut wajib dipatuhi seluruh AI Coding Assistant (Claude, Bolt.new, ChatGPT, Cursor, GitHub Copilot) tanpa pengecualian, sepanjang lifecycle proyek. **Poin 1–3 diperbarui di v1.1**; poin 32–34 ditambahkan di v1.1 untuk mengintegrasikan ADR-001; poin 35 ditambahkan di v1.2 untuk mengintegrasikan ADR-005; poin 36 baru ditambahkan di v1.3 untuk mengintegrasikan ADR-006.
...
13. Jangan mengambil keputusan sepihak untuk item berstatus "Hal Perlu Dikonfirmasi"/Open Question **atau ADR berstatus OPEN** — implementasikan sebagai configurable placeholder dengan `// TODO: menunggu resolusi ADR-XXX`, laporkan ke manusia jika memblokir progres.
...
*Dokumen ini adalah turunan operasional dari `architecture-decision-records.md`, `PROJECT-CONSTITUTION.md`, dan seluruh dokumen sumber proyek v1.1 (26–29 Juli 2026), berpasangan erat dengan `technology-decisions.md`, `dependency-manifest.md`, dan `DEVELOPMENT-ROADMAP.md`. Versi 1.1 (28 Juli 2026) mengintegrasikan ADR-001 (Backend Architecture, Approved). Versi 1.2 (28 Juli 2026) mengintegrasikan ADR-005 (Search Strategy, Approved): PostgreSQL Full-Text Search + `pg_trgm` dikunci sebagai mesin pencari Fase 1. Versi 1.3 (29 Juli 2026) mengintegrasikan ADR-006 (Job Queue Strategy, Approved): Vercel Cron Jobs + Postgres Trigger/Database Webhook dikunci sebagai mekanisme job asinkron/terjadwal Fase 1, memperbarui Bagian 4 (AI Workflow), Bagian 21 (AI Prompting Rules), Bagian 22 (Module Development), Bagian 23 (Development Order), dan Bagian 26 (AI Golden Rules). Wajib direview ulang setiap kali ada ADR baru yang disahkan atau perubahan signifikan pada dokumen governance di atasnya (lihat Bagian 1 — hierarki dokumen), dan menjadi acuan tetap bagi seluruh AI Coding Assistant serta kontributor manusia selama lifecycle proyek berlangsung.*
