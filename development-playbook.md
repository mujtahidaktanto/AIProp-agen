...
| **Last Updated** | 28 Juli 2026 — disinkronkan terhadap `architecture-decision-records.md` (ADR terbaru, khususnya **ADR-001 Backend Architecture — Approved 27 Juli 2026** dan **ADR-005 Search Strategy — Approved 28 Juli 2026**) |
...
>
> **Catatan ADR (wajib dibaca berdampingan dengan catatan di atas):** `architecture-decision-records.md` **wajib dibaca sebelum** `technology-decisions.md` (ADR Bagian 10 poin 1) karena ADR menjelaskan alasan & alternatif di balik setiap baris *Official Technology Stack*. ADR berstatus **Approved mengikat** implementasi (termasuk seluruh isi Blueprint ini); ADR berstatus **OPEN** (saat ini: ADR-006 Job Queue Strategy, ADR-008 Maps Provider, ADR-018 Caching Strategy) **tidak boleh diasumsikan** oleh AI Coding Assistant — area yang terdampak wajib diimplementasikan sebagai *configurable placeholder* dengan `// TODO: menunggu resolusi ADR-XXX` (lihat Bagian 4 & 24). **ADR-005 (Search Strategy) sudah Approved** — search Fase 1 boleh diimplementasikan penuh, lihat Bagian 22.3.
...
- **Langkah 1 (Read ADR) adalah langkah baru dan wajib paling awal** — mengikuti hierarki governance di Bagian 1: ADR berstatus **Approved** (termasuk **ADR-001 Backend Architecture**: Next.js Route Handlers + Supabase, tanpa service terpisah, dengan Bolt.new sebagai bagian toolchain resmi; dan **ADR-005 Search Strategy**: PostgreSQL FTS + `pg_trgm` Fase 1, migrasi terjadwal ke Typesense Fase 2) mengikat seluruh langkah implementasi berikutnya dan tidak boleh diperdebatkan ulang oleh AI. ADR berstatus **OPEN** (ADR-006, ADR-008, ADR-018) menandai area yang **belum boleh diasumsikan** — jika task menyentuh area ini, AI wajib mengikuti gerbang Langkah 6→7.
...
3. Task yang menyentuh item berstatus "Hal Perlu Dikonfirmasi"/Open Question, **termasuk ADR berstatus OPEN** (saat ini: ADR-006 Job Queue Strategy, ADR-008 Maps Provider, ADR-018 Caching Strategy), **wajib** diimplementasikan sebagai *configurable placeholder*, ditandai `// TODO: menunggu resolusi ADR-XXX` — AI tidak mengambil keputusan arsitektur/bisnis tsb sendiri meski task memungkinkan, dan tidak berwenang menaikkan status ADR menjadi Approved (ADR Bagian 9). **ADR-005 (Search Strategy) sudah Approved** — task pencarian listing Fase 1 tidak lagi memerlukan placeholder, lihat poin 6.
...
Panduan operasional untuk membangun modul secara individu, turunan langsung dari **ADR-001 (Backend Architecture, Approved)**, **ADR-005 (Search Strategy, Approved)**, dan `DEVELOPMENT-ROADMAP.md` (Module Order & Dependency Graph). Bagian ini menjawab pertanyaan *"bagaimana satu modul dibangun dari nol sampai selesai"* — berbeda dari Bagian 23 (Development Order) yang menjawab *"modul mana dulu"*.
...
Tiga ADR berstatus OPEN saat ini berdampak langsung pada modul tertentu (lihat ADR Bagian 7 — Impact Analysis):

| ADR OPEN | Modul Terdampak | Cara Menangani Saat Development Modul |
|---|---|---|
| **ADR-006** Job Queue Strategy | Modul 3, 5, 8, 11 (sitemap event-driven, reminder H-1, sinkronisasi counter) | Implementasikan trigger/counter yang bisa dilakukan synchronous dulu; proses async ditandai `// TODO: menunggu resolusi ADR-006`, jangan memasang BullMQ/Redis secara sepihak sebelum ADR disahkan. |
...
Modul yang **tidak** tersentuh ketiga ADR di atas (M1, M2 dasar, M3 search/filter dasar & lanjutan, M4, M7, M9/M10, M2 ext.) dapat dibangun penuh tanpa placeholder terkait ADR.
...
| 5 | Modul 11 — SEO (hardening tingkat sistem) | #2, #4 | Butuh ≥2 tipe halaman publik (profil + listing) untuk diuji bersamaan. **Catatan ADR:** sitemap event-driven menunggu ADR-006. |
...
|---|---|---|---|
| 1 (High) | **ADR-006** — Job Queue Strategy | Sprint S6 (SEO Hardening) / #5 di atas, dan Sprint S13 (Event) / #11 | Arah Edge Functions+cron menguat setelah ADR-001 dikunci — prioritas tertinggi berikutnya. |
| 2 (Medium) | **ADR-008** — Maps Provider | Sprint S4 (#4) dan S9 (#8) | Independen, dapat diputuskan kapan pun — murni menunggu konfirmasi biaya bisnis. |
| 3 (Low) | **ADR-018** — Caching Strategy | Tidak memblokir sprint manapun di roadmap saat ini | Direkomendasikan diputuskan bersamaan ADR-006 jika arah Job Queue mengarah ke BullMQ (butuh Redis). |

**Catatan penting:** ADR-001 (Backend Architecture) **telah selesai** (Approved, 27 Juli 2026) dan ADR-005 (Search Strategy) **telah selesai** (Approved, 28 Juli 2026) — keduanya tidak lagi menjadi item dalam tabel resolusi ini. Seluruh Development Order di atas kini dapat dieksekusi dengan konteks arsitektur backend dan search yang sudah pasti (Route Handlers + Supabase satu `apps/web`; PostgreSQL FTS + `pg_trgm` Fase 1 dengan migrasi terjadwal ke Typesense Fase 2; Bolt.new sebagai bagian toolchain resmi).
...
Aturan emas berikut wajib dipatuhi seluruh AI Coding Assistant (Claude, Bolt.new, ChatGPT, Cursor, GitHub Copilot) tanpa pengecualian, sepanjang lifecycle proyek. **Poin 1–3 diperbarui di v1.1**; poin 32–34 ditambahkan di v1.1 untuk mengintegrasikan ADR-001; poin 35 baru ditambahkan di v1.2 untuk mengintegrasikan ADR-005.
...
*Dokumen ini adalah turunan operasional dari `architecture-decision-records.md`, `PROJECT-CONSTITUTION.md`, dan seluruh dokumen sumber proyek v1.1 (26–28 Juli 2026), berpasangan erat dengan `technology-decisions.md`, `dependency-manifest.md`, dan `DEVELOPMENT-ROADMAP.md`. Versi 1.1 (28 Juli 2026) mengintegrasikan ADR-001 (Backend Architecture, Approved). Versi 1.2 (28 Juli 2026) mengintegrasikan ADR-005 (Search Strategy, Approved): PostgreSQL Full-Text Search + `pg_trgm` dikunci sebagai mesin pencari Fase 1, memperbarui Bagian 4 (AI Workflow), Bagian 21 (AI Prompting Rules), Bagian 22 (Module Development), Bagian 23 (Development Order), dan Bagian 26 (AI Golden Rules). Wajib direview ulang setiap kali ada ADR baru yang disahkan atau perubahan signifikan pada dokumen governance di atasnya (lihat Bagian 1 — hierarki dokumen), dan menjadi acuan tetap bagi seluruh AI Coding Assistant serta kontributor manusia selama lifecycle proyek berlangsung.*
