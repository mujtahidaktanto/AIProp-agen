...
| **Last Updated** | 27 Juli 2026 — sinkronisasi penambahan `architecture-decision-records.md` (ADR-001 Backend Architecture, Approved) ke Baseline Register, Version Matrix, Review Schedule, Source of Truth Matrix, Document Status, dan Dependency Matrix; nomor versi dokumen ini **tidak naik** (masih Draft — lihat Bagian 5, "nomor versi hanya naik saat memasuki status Baseline"). |
...
| **Arsitektur & Keputusan Teknis per Topik (ADR)** | `architecture-decision-records.md` — **25 ADR** (`ADR-001`–`ADR-025`), disusun per topik arsitektur (Backend, Auth, RBAC, Database, Search, Job Queue, Email, Maps, Storage, Deployment, State Management, API Architecture, Error Handling, Logging, Monitoring, Testing, Security, Caching, File Upload, Notification, Frontend Framework, Schema Conventions, Multi-Tenancy, RBAC Role Scope, Type Safety). **Dibaca sebelum** `technology-decisions.md` — menjelaskan alasan di balik apa yang tercantum di sana. | `technology-decisions.md`, `SYSTEM-ARCHITECTURE.md` §4, `AI-DEVELOPMENT-BLUEPRINT.md`, `dependency-manifest.md` |
...
Architecture Decision Records (architecture-decision-records.md — 25 ADR per topik arsitektur,
        termasuk ADR-001 Backend Architecture, Approved 27 Jul 2026)
        ↓
...
> Data diisi berdasarkan isi aktual dokumen yang tersedia pada tanggal penyusunan (27 Juli 2026). Kolom yang informasinya tidak tersedia secara eksplisit di dokumen sumber ditandai **TBD**. **Disinkronkan ulang 27 Juli 2026** dengan masuknya `architecture-decision-records.md` (ADR terbaru: `ADR-001` Backend Architecture, Approved) dan rilis `CHANGELOG.md` `0.1.1` / `decision-log.md` `ADR-038` — baris terdampak ditandai eksplisit di kolom **Last Review**, bukan diedit diam-diam.

| Document | Current Version | Status | Baseline Version | Owner | Last Review | Next Review | Dependencies | Source of Truth Untuk |
|---|---|---|---|---|---|---|---|---|
| PROJECT-CONSTITUTION.md | 1.1 | Baseline (dinyatakan "BERLAKU") | 1.1 | Principal Software Architect (nama TBD) | 26 Jul 2026 (revisi v1.0→v1.1) | Setiap keputusan bisnis besar turun | — (tertinggi) | Governance / Engineering Guidelines |
...
| SYSTEM-ARCHITECTURE.md | 1.0 | Approved ("Referensi teknis utama — mengikat"), **belum Baseline formal** karena masih memuat beberapa frasa usang (§4, §10, §23 — lihat Governance Notes) | TBD (menunggu sinkronisasi) | Enterprise Solution Architect (nama TBD) | 27 Jul 2026 | Setelah Open Decision H1–H3 (`foundation-validation-report.md` §16) diselesaikan | PROJECT-CONSTITUTION.md, dokumen sumber v1.1 | Architecture (High-Level) |
| architecture-decision-records.md | 1.0 | **Draft** (status dokumen sendiri) — meski `ADR-001` (Backend Architecture) di dalamnya sudah berstatus entry-level **Approved** (hasil Architecture Review Board, 27 Jul 2026). 24 ADR lain bercampur status Approved/Open/Rejected/Superseded per-entry — lihat dokumen tsb Bagian 5 (Open Decisions Summary). | Belum ada (blocked oleh status dokumen Draft — sama seperti pola `technology-decisions.md`) | Principal Enterprise Software Architect / Enterprise Solution Architect (nama TBD) | 27 Jul 2026 (ADR-001 dikunci Approved) | Setelah pengesahan tim formal + seluruh ADR ber-status Open (ADR-005 Search, ADR-006 Job Queue) diselesaikan | PROJECT-CONSTITUTION.md, dokumen sumber v1.1 | Arsitektur & Keputusan Teknis per Topik (ADR) |
| technology-decisions.md | 1.0 | **Draft** (status dokumen sendiri) — meski isinya disebut "keputusan resmi dan final" | Belum ada (blocked oleh status Draft) | Principal Software Architect / Technical Lead (nama TBD) | 27 Jul 2026 | Setelah pengesahan tim + Open Questions §9 diselesaikan **dan** disinkronkan dengan `architecture-decision-records.md` `ADR-001` (penambahan **Bolt.new** sebagai toolchain resmi — catatan kondisional Board, belum dieksekusi) | SYSTEM-ARCHITECTURE.md, PROJECT-CONSTITUTION.md, **architecture-decision-records.md** (baru) | Technology Stack & Rasionalisasi |
| dependency-manifest.md | 1.0 (tersirat, tidak eksplisit dicantumkan sebagai nomor versi terpisah) | Draft (mengikuti status technology-decisions.md) | Belum ada | Principal Software Architect (nama TBD) | 27 Jul 2026 | Bersamaan dengan technology-decisions.md | technology-decisions.md | Dependency/Package Katalog |
| AI-DEVELOPMENT-BLUEPRINT.md (versi upload, 24 bagian) | 1.0 | Draft (status dokumen), namun **ditetapkan sebagai acuan aktif** oleh keputusan user (CHANGELOG Session 5) menggantikan versi Session 3 (32 bagian, kini Archived) | Belum ada Baseline formal | Principal Software Architect (nama TBD) | 27 Jul 2026 | Saat AI Workflow/Rules berubah | Seluruh dokumen sumber v1.1 + System Architecture + Technology Decisions | AI Instruction / Development Playbook |
...
| CURRENT-PROJECT-STATE.md | 0.1 | Baseline (Living Document — berlaku sejak dibuat, wajib update tiap sesi) | 0.1 (per-sesi, berubah dinamis) | Technical Project Manager (nama TBD) | 27 Jul 2026 | Akhir setiap sesi development | Kondisi fisik repositori (independen dokumen desain) | Status Implementasi Nyata |
| CHANGELOG.md | 0.1.1 (versi rilis proyek yang dicatat di dalamnya; dokumen itu sendiri tidak diberi nomor versi terpisah) | Baseline (Living Document) | 0.1.1 | Release Manager (nama TBD) | 27 Jul 2026 (rilis `0.1.1` — Governance Sync: `architecture-decision-records.md` ditambahkan, Known Issue #1 ditandai RESOLVED) | Setiap rilis versi baru | CURRENT-PROJECT-STATE.md | History |
| decision-log.md | 1.0 | Baseline (Living Document — "BERLAKU sejak dibuat") | 1.0 (per-ADR, berkembang) | Principal Software Architect / Technical Lead (nama TBD) | 27 Jul 2026 (entry `ADR-038` ditambahkan — sinkronisasi `architecture-decision-records.md` `ADR-001` Backend Architecture) | Berkelanjutan (tiap ADR baru) | Seluruh dokumen governance, **architecture-decision-records.md** (baru, sebagai sumber sinkronisasi ADR-038) | Decision (jurnal kronologis) |
...
| document-governance-baseline-register.md (dokumen ini) | 1.0 | Draft | Belum ada | Software Configuration Manager (nama TBD) | 27 Jul 2026 | Setiap kali status/versi dokumen lain berubah material | Seluruh dokumen di atas | Tata Kelola Dokumen |
...
- [ ] Tidak ada **Open Decision** yang memblokir modul yang akan dikerjakan — cek `decision-log.md` Bagian 11 dan `CHANGELOG.md` Known Issues sebelum memulai setiap sprint — **saat ini masih tercatat 8 baris di `decision-log.md` §11, namun secara substansi tinggal 7 yang benar-benar terbuka**: butir #1 (arsitektur backend) sudah dikunci **Approved** via `architecture-decision-records.md` `ADR-001` dan disinkronkan sebagai `decision-log.md` `ADR-038` (27 Jul 2026), `CHANGELOG.md` Known Issue #1 sudah ditandai `RESOLVED (0.1.1)` — namun baris #1 di `decision-log.md` §11 itu sendiri **belum diformat ulang** menjadi status Resolved (masih tertulis apa adanya, sesuai aturan "tidak menghapus histori"). Sebagian sisanya bersifat blocking untuk sprint tertentu (lihat tabel di `decision-log.md` §11).
- [ ] **Decision Log** dalam kondisi terbaru — seluruh keputusan yang relevan dengan sprint yang akan dikerjakan sudah tercatat sebagai ADR. **Terkini**: entry `ADR-038` (27 Jul 2026) sudah menyinkronkan keputusan Backend Architecture dari `architecture-decision-records.md` `ADR-001`.
- [ ] **Changelog** dalam kondisi terbaru — versi proyek saat ini (`0.1.1`) mencerminkan kondisi nyata repositori (rilis Governance Sync: penambahan `architecture-decision-records.md` + sinkronisasi `decision-log.md`).
