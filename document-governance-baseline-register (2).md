...
| **Last Updated** | 28 Juli 2026 — sinkronisasi penambahan resolusi `architecture-decision-records.md` `ADR-005` (Search Strategy, Approved) ke Baseline Register, Version Matrix, Review Schedule, Source of Truth Matrix, Document Status, dan Dependency Matrix; versi terbaru dokumen turunan (`technology-decisions.md` v1.2, `SYSTEM-ARCHITECTURE.md` v1.2, `dependency-manifest.md` v1.2, `PROJECT-CONSTITUTION.md` v1.3, `AI-DEVELOPMENT-BLUEPRINT.md` v1.2, `CHANGELOG.md` 0.1.2, `decision-log.md` entry `ADR-039`) direfleksikan penuh di Bagian 10. Nomor versi dokumen ini **tidak naik** (masih Draft — lihat Bagian 5, "nomor versi hanya naik saat memasuki status Baseline"). |
...
| **Arsitektur & Keputusan Teknis per Topik (ADR)** | `architecture-decision-records.md` — **25 ADR** (`ADR-001`–`ADR-025`), disusun per topik arsitektur (Backend, Auth, RBAC, Database, Search, Job Queue, Email, Maps, Storage, Deployment, State Management, API Architecture, Error Handling, Logging, Monitoring, Testing, Security, Caching, File Upload, Notification, Frontend Framework, Schema Conventions, Multi-Tenancy, RBAC Role Scope, Type Safety). **Status per 28 Jul 2026: 22 Approved** (termasuk `ADR-001` Backend Architecture & `ADR-005` Search Strategy), **3 OPEN** (`ADR-006` Job Queue, `ADR-008` Maps, `ADR-018` Caching). Dibaca sebelum `technology-decisions.md` — menjelaskan alasan di balik apa yang tercantum di sana. | `technology-decisions.md`, `SYSTEM-ARCHITECTURE.md` §4, `AI-DEVELOPMENT-BLUEPRINT.md`, `dependency-manifest.md` |
...
Architecture Decision Records (architecture-decision-records.md — 25 ADR per topik arsitektur,
        22 Approved termasuk ADR-001 Backend Architecture & ADR-005 Search Strategy,
        3 OPEN: ADR-006, ADR-008, ADR-018)
        ↓
...
> Data diisi berdasarkan isi aktual dokumen yang tersedia pada tanggal penyusunan (27 Juli 2026), **disinkronkan ulang 28 Juli 2026** dengan resolusi `architecture-decision-records.md` `ADR-005` (Search Strategy, Approved) dan rilis `CHANGELOG.md` `0.1.2` / `decision-log.md` `ADR-039`. Kolom yang informasinya tidak tersedia secara eksplisit di dokumen sumber ditandai **TBD**. Baris terdampak ditandai eksplisit di kolom **Last Review**, bukan diedit diam-diam.

| Document | Current Version | Status | Baseline Version | Owner | Last Review | Next Review | Dependencies | Source of Truth Untuk |
|---|---|---|---|---|---|---|---|---|
| PROJECT-CONSTITUTION.md | 1.3 | Baseline (dinyatakan "BERLAKU") | 1.3 | Principal Software Architect (nama TBD) | 28 Jul 2026 (revisi v1.2→v1.3, sinkron `ADR-005`) | Setiap keputusan bisnis besar turun | — (tertinggi) | Governance / Engineering Guidelines |
...
| SYSTEM-ARCHITECTURE.md | 1.2 | Approved ("Referensi teknis utama — mengikat"), **belum Baseline formal** (menunggu pengesahan nama individu Reviewer/Approver) | TBD (menunggu sinkronisasi) | Enterprise Solution Architect (nama TBD) | 28 Jul 2026 (naik v1.1→v1.2, sinkron `ADR-005`) | Setelah Open Decision H1–H3 (`foundation-validation-report.md` §16) diselesaikan | PROJECT-CONSTITUTION.md, dokumen sumber v1.1, **architecture-decision-records.md** | Architecture (High-Level) |
| architecture-decision-records.md | 1.0 | **Draft** (status dokumen sendiri) — meski `ADR-001` (Backend Architecture) dan `ADR-005` (Search Strategy) di dalamnya sudah berstatus entry-level **Approved** (hasil Architecture Review Board, 27 & 28 Jul 2026). 23 ADR lain bercampur status Approved/Open/Rejected/Superseded per-entry — lihat dokumen tsb Bagian 5 (Open Decisions Summary). | Belum ada (blocked oleh status dokumen Draft — sama seperti pola `technology-decisions.md`) | Principal Enterprise Software Architect / Enterprise Solution Architect (nama TBD) | 28 Jul 2026 (ADR-005 dikunci Approved) | Setelah pengesahan tim formal + seluruh ADR ber-status Open (ADR-006 Job Queue, ADR-008 Maps, ADR-018 Caching) diselesaikan | PROJECT-CONSTITUTION.md, dokumen sumber v1.1 | Arsitektur & Keputusan Teknis per Topik (ADR) |
| technology-decisions.md | 1.2 | **Draft** (status dokumen sendiri) — meski isinya disebut "keputusan resmi dan final" | Belum ada (blocked oleh status Draft) | Principal Software Architect / Technical Lead (nama TBD) | 28 Jul 2026 (naik v1.1→v1.2, sinkron `ADR-005`) | Setelah pengesahan tim + Open Questions §9 diselesaikan **dan** disinkronkan dengan `architecture-decision-records.md` `ADR-006`/`ADR-008`/`ADR-018` (masih Open); penambahan **Bolt.new** sebagai toolchain resmi (catatan kondisional Board `ADR-001`) masih belum dieksekusi | SYSTEM-ARCHITECTURE.md, PROJECT-CONSTITUTION.md, **architecture-decision-records.md** | Technology Stack & Rasionalisasi |
| dependency-manifest.md | 1.2 | Draft (mengikuti status technology-decisions.md) | Belum ada | Principal Software Architect (nama TBD) | 28 Jul 2026 (naik v1.1→v1.2, sinkron `ADR-005` — tidak ada package npm baru, hanya klarifikasi ekstensi Postgres) | Bersamaan dengan technology-decisions.md | technology-decisions.md | Dependency/Package Katalog |
| AI-DEVELOPMENT-BLUEPRINT.md (versi upload, 24 bagian) | 1.2 | Draft (status dokumen), namun **ditetapkan sebagai acuan aktif** oleh keputusan user (CHANGELOG Session 5) menggantikan versi Session 3 (32 bagian, kini Archived) | Belum ada Baseline formal | Principal Software Architect (nama TBD) | 28 Jul 2026 (naik v1.1→v1.2, sinkron `ADR-005`) | Saat AI Workflow/Rules berubah | Seluruh dokumen sumber v1.1 + System Architecture + Technology Decisions | AI Instruction / Development Playbook |
...
| CURRENT-PROJECT-STATE.md | 0.1 | Baseline (Living Document — berlaku sejak dibuat, wajib update tiap sesi) | 0.1 (per-sesi, berubah dinamis) | Technical Project Manager (nama TBD) | 28 Jul 2026 (snapshot ADR & Governance diperbarui, sinkron `ADR-005`) | Akhir setiap sesi development | Kondisi fisik repositori (independen dokumen desain) | Status Implementasi Nyata |
| CHANGELOG.md | 0.1.2 (versi rilis proyek yang dicatat di dalamnya; dokumen itu sendiri tidak diberi nomor versi terpisah) | Baseline (Living Document) | 0.1.2 | Release Manager (nama TBD) | 28 Jul 2026 (rilis `0.1.2` — Governance Sync: `ADR-005` Search Strategy diselesaikan, Known Issue #5 bagian Search Engine ditandai RESOLVED) | Setiap rilis versi baru | CURRENT-PROJECT-STATE.md | History |
| decision-log.md | 1.0 | Baseline (Living Document — "BERLAKU sejak dibuat") | 1.0 (per-ADR, berkembang) | Principal Software Architect / Technical Lead (nama TBD) | 28 Jul 2026 (entry `ADR-039` ditambahkan — sinkronisasi `architecture-decision-records.md` `ADR-005` Search Strategy) | Berkelanjutan (tiap ADR baru) | Seluruh dokumen governance, **architecture-decision-records.md** (sebagai sumber sinkronisasi ADR-038 & ADR-039) | Decision (jurnal kronologis) |
...
| document-governance-baseline-register.md (dokumen ini) | 1.0 | Draft | Belum ada | Software Configuration Manager (nama TBD) | 28 Jul 2026 | Setiap kali status/versi dokumen lain berubah material | Seluruh dokumen di atas | Tata Kelola Dokumen |
...
- [ ] Tidak ada **Open Decision** yang memblokir modul yang akan dikerjakan — cek `decision-log.md` Bagian 11 dan `CHANGELOG.md` Known Issues sebelum memulai setiap sprint — **saat ini masih tercatat 8 baris di `decision-log.md` §11, namun secara substansi tinggal 6 yang benar-benar terbuka**: butir #1 (arsitektur backend) sudah dikunci **Approved** via `architecture-decision-records.md` `ADR-001`/`decision-log.md` `ADR-038` (27 Jul 2026); butir #5 (bagian Search Engine) sudah dikunci **Approved** via `ADR-005`/`ADR-039` (28 Jul 2026, bagian Job Queue pada butir #5 tetap terbuka) — `CHANGELOG.md` Known Issue #1 ditandai `RESOLVED (0.1.1)` dan bagian Search Engine pada Known Issue #5 ditandai `RESOLVED (0.1.2)`. Baris #1 dan #5 di `decision-log.md` §11 itu sendiri **belum diformat ulang** sepenuhnya menjadi status Resolved terpisah (masih memakai strikethrough sebagian, sesuai aturan "tidak menghapus histori"). Sebagian sisanya bersifat blocking untuk sprint tertentu (lihat tabel di `decision-log.md` §11).
- [ ] **Decision Log** dalam kondisi terbaru — seluruh keputusan yang relevan dengan sprint yang akan dikerjakan sudah tercatat sebagai ADR. **Terkini**: entry `ADR-039` (28 Jul 2026) sudah menyinkronkan keputusan Search Strategy dari `architecture-decision-records.md` `ADR-005`.
- [ ] **Changelog** dalam kondisi terbaru — versi proyek saat ini (`0.1.2`) mencerminkan kondisi nyata repositori (rilis Governance Sync: resolusi `ADR-005` + sinkronisasi berantai ke `technology-decisions.md`, `SYSTEM-ARCHITECTURE.md`, `dependency-manifest.md`, `PROJECT-CONSTITUTION.md`, `AI-DEVELOPMENT-BLUEPRINT.md`).
