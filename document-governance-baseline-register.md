...
| **Related Documents** | Seluruh 18 dokumen proyek yang terdaftar di Bagian 10 (Baseline Register) |
...
| **Keputusan & Rasionalisasi (ADR)** | `decision-log.md` | Seluruh dokumen lain yang merujuk alasan keputusan |
...
> Data diisi berdasarkan isi aktual dokumen yang tersedia pada tanggal penyusunan (27 Juli 2026). Kolom yang informasinya tidak tersedia secara eksplisit di dokumen sumber ditandai **TBD**.
...
| technology-decisions.md | 1.0 | **Draft** (status dokumen sendiri) — meski isinya disebut "keputusan resmi dan final" | Belum ada (blocked oleh status Draft) | Principal Software Architect / Technical Lead (nama TBD) | 27 Jul 2026 | Setelah pengesahan tim + Open Questions §9 diselesaikan | SYSTEM-ARCHITECTURE.md, PROJECT-CONSTITUTION.md | Technology Stack & Rasionalisasi |
...
| CHANGELOG.md | 0.1.0 (versi rilis proyek yang dicatat di dalamnya; dokumen itu sendiri tidak diberi nomor versi terpisah) | Baseline (Living Document) | 0.1.0 | Release Manager (nama TBD) | 27 Jul 2026 | Setiap rilis versi baru | CURRENT-PROJECT-STATE.md | History |
| decision-log.md | 1.0 | Baseline (Living Document — "BERLAKU sejak dibuat") | 1.0 (per-ADR, berkembang) | Principal Software Architect / Technical Lead (nama TBD) | 27 Jul 2026 | Berkelanjutan (tiap ADR baru) | Seluruh dokumen governance | Decision |
...
7. **Ikuti hierarki kemenangan dokumen** (`PROJECT-CONSTITUTION.md` > dokumen sumber v1.1 > `SYSTEM-ARCHITECTURE.md` > `technology-decisions.md` > `dependency-manifest.md` > `AI-DEVELOPMENT-BLUEPRINT.md`) setiap kali dua dokumen tampak memberi instruksi berbeda.
...
- [ ] Tidak ada **Open Decision** yang memblokir modul yang akan dikerjakan — cek `decision-log.md` Bagian 11 dan `CHANGELOG.md` Known Issues sebelum memulai setiap sprint — **saat ini masih ada 8 Open Decision aktif**, sebagian bersifat blocking untuk sprint tertentu (lihat tabel di `decision-log.md` §11).
- [ ] **Decision Log** dalam kondisi terbaru — seluruh keputusan yang relevan dengan sprint yang akan dikerjakan sudah tercatat sebagai ADR.
- [ ] **Changelog** dalam kondisi terbaru — versi proyek saat ini (`0.1.0`) mencerminkan kondisi nyata repositori.
- [ ] **Dependency antar dokumen konsisten** — tidak ada dokumen turunan yang mengklaim sesuatu "final" sementara dokumen sumbernya masih mencatat "terbuka" (lihat Governance Notes Bagian 15 untuk daftar ketidaksesuaian yang masih berjalan).
- [ ] **Source of Truth jelas** untuk setiap topik yang akan disentuh sprint — verifikasi lewat Bagian 7 dokumen ini sebelum menulis kode.
...
- [ ] **Nama individu Owner/Reviewer/Approver** ditetapkan untuk setiap dokumen di Bagian 9 & 10 — saat ini seluruhnya masih diisi peran, bukan nama (TBD).
