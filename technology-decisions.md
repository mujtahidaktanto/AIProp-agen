...
**Kedudukan dokumen dalam hierarki governance proyek:** Dokumen ini adalah turunan operasional dari `PROJECT-CONSTITUTION.md` Bagian 4 (Tech Stack & Framework) dan `SYSTEM-ARCHITECTURE.md` Bagian 4 (Technology Stack). Jika terjadi ketidaksesuaian, **`PROJECT-CONSTITUTION.md` yang menang**. Dokumen ini **tidak menggantikan** Constitution — fungsinya adalah memperdalam *alasan* di balik setiap pilihan teknologi resmi agar konsisten dipahami oleh seluruh AI Coding Assistant (Claude, Bolt.new, ChatGPT, Cursor) dan developer manusia.

**Dokumen sumber yang menjadi rujukan:** `PROJECT-CONSTITUTION.md`, `SYSTEM-ARCHITECTURE.md`, `AI-DEVELOPMENT-BLUEPRINT.md`, `AI-CONTEXT-PACK.md`, `PRD-Real-Estate-Agency-Platform-v1.1.md`, `ERD-Skema-Database-Real-Estate-Agency-v1.1.md`, `API-Specification-Real-Estate-Agency-Platform-v1.1.md`, `SEO-Analytics-Specification-Real-Estate-Agency-Platform-v1.1.md`.
...
Tabel berikut adalah **keputusan resmi dan final** proyek — identik dengan `PROJECT-CONSTITUTION.md` Bagian 4, tidak boleh disimpangi tanpa keputusan arsitektur eksplisit yang disetujui.
...
> **Catatan penting terhadap dokumen sumber:** `PROJECT-CONSTITUTION.md` Bagian 4 masih menampilkan **backend/API** sebagai dua opsi ("Next.js Route Handlers" **atau** "service backend terpisah NestJS/Express") yang harus dikunci eksplisit sebelum Fase 1 selesai. Dengan menetapkan **Backend → Supabase** secara eksplisit di sini, keputusan tersebut secara efektif **terselesaikan ke arah opsi pertama**: backend logic berjalan sebagai Next.js Route Handlers (BFF tipis) yang memanggil Supabase langsung (client SDK di server + service role key untuk operasi admin), **tanpa** service backend Node.js terpisah (NestJS/Express). Ini **wajib** dikonfirmasi dan disinkronkan balik ke `PROJECT-CONSTITUTION.md` Bagian 4 & 23 serta `SYSTEM-ARCHITECTURE.md` Bagian 21/23, karena kedua dokumen tersebut masih mencatatnya sebagai *open question* — lihat Bagian 9 (Open Questions) dokumen ini.
...
> **Catatan governance:** sesuai `AI-CONTEXT-PACK.md` bagian "Potential Conflict" poin 3, Vercel sebagai hosting resmi **belum tercatat formal** di `PROJECT-CONSTITUTION.md` sebagai keputusan arsitektur (hanya disebut di `SYSTEM-ARCHITECTURE.md`). Instruksi pembuatan dokumen ini secara eksplisit menetapkan Vercel sebagai keputusan resmi — item ini perlu disinkronkan balik ke Constitution (lihat Bagian 9).
...
### 4.29 Google Maps Platform — Maps
- **Purpose:** Autocomplete alamat (client-side), reverse geocoding & distance matrix (server-side) untuk fitur lokasi listing.
- **Why Selected:** Ditetapkan eksplisit sebagai keputusan resmi di Bagian 3 dokumen ini — menyelesaikan status "belum final" antara Google Maps Platform vs Mapbox yang masih tercatat di `PROJECT-CONSTITUTION.md`/`API-Specification` §13/9.1 sebagai open question.
...
- **Alternative Considered:** Mapbox.
- **Why Alternative Was Rejected:** Mapbox tetap merupakan alternatif valid secara teknis (dan pernah dicatat sebagai opsi terbuka di dokumen sumber), namun instruksi pembuatan dokumen ini secara eksplisit menetapkan Google Maps Platform — **keputusan ini perlu disinkronkan balik** ke `PROJECT-CONSTITUTION.md`/`AI-CONTEXT-PACK.md` yang masih mencatat pilihan provider Maps sebagai "Hal Perlu Dikonfirmasi" (lihat Bagian 9, Open Questions).
- **Integration Notes:** Reverse geocoding & Distance Matrix dipanggil server-side (API key rahasia); Autocomplete client-side (API key dibatasi domain/referrer).
- **AI Development Notes:** Jangan pernah mengekspos `GOOGLE_MAPS_API_KEY_SERVER` ke client — hanya `GOOGLE_MAPS_API_KEY_CLIENT` yang boleh dibundel ke frontend.
...
10. **Tidak boleh menambahkan backend service Node.js terpisah** (NestJS/Express, dsb.) tanpa Architecture Decision eksplisit — Backend resmi adalah Supabase + Next.js Route Handlers (lihat catatan Bagian 3).
...
5. **Ikuti seluruh keputusan dokumen ini** dan `PROJECT-CONSTITUTION.md` — jika keduanya tampak berbeda, `PROJECT-CONSTITUTION.md` yang menang; laporkan sebagai `// TODO: konflik technology-decisions vs constitution`.
...
|---|---|---|
| **Redis** | Cache & rate limiting | Jika rate limiting endpoint sensitif (auth/OTP) & cache edge Vercel/Supabase bawaan dinilai belum cukup saat traffic bertumbuh (lihat Bagian 9 poin 3 — saat ini masih *gap* yang perlu keputusan lebih awal, bukan murni "fase depan"). |
| **Queue** | BullMQ (butuh Redis) atau Supabase Edge Functions + cron | Untuk regenerasi sitemap event-driven, sinkronisasi counter denormalisasi, pengiriman notifikasi terjadwal (lihat Bagian 9 poin 4). |
| **Search Engine** | Typesense (direkomendasikan dokumen sumber) atau Elasticsearch | Untuk `/properties/search` & `/properties/autocomplete` dengan typo-tolerance — saat ini juga masih *gap* di Official Stack, lihat Bagian 9 poin 2. |
...
Sesuai instruksi pembuatan dokumen ini — **tidak ada asumsi yang dibuat** untuk poin-poin berikut; seluruhnya wajib dikonfirmasi tim sebelum atau selama development, dan diimplementasikan sebagai *configurable placeholder* dengan penanda `// TODO` di kode:

1. **Arsitektur backend/API kini condong ke "Supabase + Next.js Route Handlers"** (tanpa service Node.js terpisah) sebagai konsekuensi logis dari Bagian 3 menetapkan "Backend → Supabase" tanpa entri backend framework terpisah. Namun `PROJECT-CONSTITUTION.md` Bagian 4 & `SYSTEM-ARCHITECTURE.md` Bagian 23 poin 1 masih mencatat ini sebagai keputusan terbuka (NestJS/Express vs Route Handlers) yang harus "dikunci sebelum Fase 1 selesai". **Perlu konfirmasi eksplisit tim** apakah pembacaan ini benar, lalu disinkronkan balik ke Constitution & System Architecture agar tidak ada dua dokumen yang saling bertentangan status "final"-nya.
2. **Search Engine belum ada di Official Technology Stack** (Bagian 3), padahal `API-Specification` mensyaratkan `/properties/search` dan `/properties/autocomplete` dengan typo-tolerance yang secara teknis membutuhkan mesin pencari khusus (Typesense/Elasticsearch direkomendasikan dokumen sumber) — **query database Postgres murni (full-text/trigram index) kemungkinan tidak cukup** untuk kebutuhan ini pada volume listing besar. Perlu keputusan eksplisit: apakah cukup mengandalkan Postgres full-text search di awal, atau menambah Typesense sejak Fase 1.
3. **Cache/Rate limiting belum ada di Official Technology Stack**, padahal `PROJECT-CONSTITUTION.md` Bagian 20 poin 6 mewajibkan rate limiting endpoint auth/OTP untuk mencegah brute-force. Perlu keputusan: menggunakan fitur rate-limiting bawaan Vercel/edge middleware, atau tetap menambah Redis seperti direkomendasikan dokumen sumber.
4. **Job Queue/scheduled job belum ada di Official Technology Stack**, padahal beberapa proses wajib bersifat asinkron/terjadwal: regenerasi sitemap event-driven, sinkronisasi counter denormalisasi (`total_listings_sold`, `cta_click_count`), pemanggilan Google Indexing API. Perlu keputusan: Supabase Edge Functions + cron (tanpa Redis tambahan) atau BullMQ (butuh Redis).
5. **Vercel sebagai hosting resmi** digunakan di dokumen ini sesuai instruksi eksplisit pembuatan dokumen, namun **belum tercatat sebagai keputusan arsitektur formal** di `PROJECT-CONSTITUTION.md` (baru muncul di `SYSTEM-ARCHITECTURE.md`) — perlu ditambahkan ke Constitution Bagian 4 agar tidak menjadi asumsi implisit yang berbeda antar dokumen (dicatat juga di `AI-CONTEXT-PACK.md` "Potential Conflict" poin 3).
6. **Google Maps Platform ditetapkan sebagai provider Maps resmi** di dokumen ini, menyelesaikan status "belum final" (vs Mapbox) yang masih tercatat di `PROJECT-CONSTITUTION.md`, `SYSTEM-ARCHITECTURE.md`, dan `API-Specification` §13. Perlu konfirmasi eksplisit tim bisnis (implikasi biaya per-request) sebelum keputusan ini disinkronkan balik ke dokumen sumber sebagai final.
7. **Package/library wrapper spesifik untuk Google Maps Platform di React** (mis. `@vis.gl/react-google-maps` — wrapper resmi Google — vs `@react-google-maps/api` yang lebih lama) belum ditentukan; lihat catatan di `dependency-manifest.md` Bagian 9.
8. **Threshold DBR final, model monetisasi, kebijakan eksklusivitas developer per wilayah, kepemilikan akun GTM/GSC/GA4** — seluruhnya tetap berstatus terbuka sesuai `PROJECT-CONSTITUTION.md` (bukan konflik teknologi, melainkan keputusan bisnis yang memang belum diambil); tidak memengaruhi Official Technology Stack di dokumen ini, namun tetap wajib diimplementasikan sebagai *configurable* (`system_configs`/`dbr_config`).
9. **Owner/penanggung jawab individu dokumen ini** (Bagian 1) belum ditentukan sebagai nama spesifik — saat ini hanya diisi peran (Principal Software Architect/Technical Lead).

---

*Dokumen ini disusun sebagai referensi teknologi resmi turunan dari `PROJECT-CONSTITUTION.md` v1.1 dan seluruh dokumen sumber proyek (26–27 Juli 2026). Berpasangan dengan `dependency-manifest.md`. Wajib direview ulang setiap kali ada keputusan pada Bagian 9 (Open Questions) yang diselesaikan, dan perubahan wajib disinkronkan balik ke `PROJECT-CONSTITUTION.md` & `SYSTEM-ARCHITECTURE.md`.*
